...
Current baseline (2026-06-01 **S153** — **`<each>`-in-dynamic-context fix sweep: 4 codegen/parser fixes + maps refresh; HIGH stays 0**. The S152 dogfood left req2 (the engine-variation todo) mounting clean but with an empty list; S153 fixed that and swept every place an `<each>` lives inside a *dynamic* mount: **engine-gated `<each>` populate** [`54d54d4d` — the req2 blocker; 3 coupled codegen modes: read `_items` BEFORE the `!_mount` early-return so the one-shot dep pass records the dep, a decoupled `_scrml_each_renderers` registry + `_scrml_remount_each` the variant-swap dispatcher calls, + `detectRuntimeChunks` descends into engine `bodyChildren`] + **`<each>` in a block-form `<match>` arm** [`3429b385` — REAL pre-existing bug: each-bearing arms re-parse via `splitBlocks`+`buildAST` so `@.` scopes; the S152 #1 "covers block-form match" claim was aspirational — now real] + **`:`-shorthand child in an engine arm** [`c89c1cb1` — parser; `<li : @.name>` inside an engine state-child broke closer-pairing → `E-ENGINE-STATE-CHILD-MISSING`; attr-aware `isColonShorthandOpener` in the 3 engine-statechild closer-finders] + **`<each>` over an enclosing-scope binding** [`e6870f25` — nested-each + each-in-component; inline emission in the outer factory + 3 component-expander roots] + **maps refresh** [`efcd5536` — S148-S152 catch-up]. req2 now renders end-to-end. NO design ratifications — the surfaced design questions were parked for user (`:`-shorthand-on-lowercase-HTML render, §4.14/§51.0.I `:`-placement discrepancy, scalar/struct zero-default). **NEW hard M5-swap precondition, witnessed twice:** the native parser does NOT promote `<each>`/`<match>` to structural nodes — two fixes this session route around it via the legacy path. Session ended **mid-wrap on power loss**; wrap completed next session from intact local state (nothing lost). On top of v0.7.0, no version cut; full suite **22,586 pass / 0 fail / 220 skip / 873 files**; known-gaps **HIGH 0**. PRIOR — 2026-06-01 **S152** — **dogfood-driven fix chain: 5 fixes + a grammar feature, HIGH back to 0**. Writing a real todo two ways (`masterScrml/req.scrml` + `req2.scrml` — the scrml rendering of TJ DeVries' CLI-todo language) surfaced and closed a chain of real gaps: **#7 `<each>` body interactivity** [`9c192c73` — `@.` in attr-values + `class:`/handlers/`${}` wired] + **scrml dev fixes** [`efc23ecf` — per-file watcher kills the recursive-node_modules inotify-ENOSPC crash + graceful degrade + root-`/` entry-preference; THIS + #6 were the actual "nothing renders"] + **#6 cross-file CLIENT module-loading** [`5082ff3c` — `_scrml_modules` registry, Approach B per the `client-cross-file-module-loading-2026-06-01` deep-dive; was emitting raw ES `import` into a classic `<script>`] + **inline `?{}` in a conditional branch** [`893872e3` — CPS boundary-recognition recurses into `match`-arm / `if` bodies; coupled fix closed a match-server-emit **`_scrml_sql`-leak-into-client** (E-CG-006)] + **SPEC §6.2 Shape 4** [`46f9bb55` — typed-array no-RHS `<x>: T[]` → `[]`; non-array no-RHS → new `E-DECL-NEEDS-INITIALIZER`; closes the no-init-undefined hole] + **each-render-before-cell-init crash** [`25a1c243` — HIGH; the `<each>` render fired at module-init BEFORE the cell-init → `_scrml_reconcile_list(undefined)` crash on ANY no-`<empty>` `<each>` over a same-program cell; fix = defer each-dispatchers after cell-init + undefined guards; #7-test blind spot = all each-tests carried an `<empty>` guard that masked it]. 3 design ratifications: the **`---` answer-delimiter convention** (→ pa.md), **DQ-2 conceded** (array mutation methods don't signal reactivity — reassignment-canonical stands, no proxy-reactive-array reopen), **Shape 4**. NEW open: **engine-gated-`<each>`-populate** (MED — req2 mounts but won't populate; the each-render skips dep-tracking when its mount is engine-variant-gated), the **A-4 atom-emitter** bare-import follow-up, the **scalar/struct zero-default** Q, and the conditional-tier/multi-batch CPS debt (the body-split's remaining envelope, now well-characterized). On top of v0.7.0, no version cut; known-gaps **HIGH 0**. PRIOR — 2026-06-01 **S151** — **C1 self-demo website inc1 LANDED** [`c66af6b2` — the compile-transparent viewer in `docs/website-viewer/` (SIBLING; the 97-page site provably untouched); 60/40 layout = live mario flagship iframe + stacked source / engine what-comes-next / tabbed JS·HTML·CSS; **REAL byte-identical `.js.map` bidirectional hover-provenance — NOT a 0:0 stub** + engine box from `--emit-engine-graph`; built AS a scrml app; serve via `serve.sh`; serve-before-push overridden by explicit user "commit C1" on the verification] + **C4/R28-5 object-literal lifecycle E-TYPE-001 dormancy RESOLVED** [`cce289b4` — Path 4 in `collectStructBindings` reusing `seedInitialFromObjectLiteral`; +10 tests; given-guard struct-field discrimination filed NEW LOW as pre-existing] + **R28-C2 kickstarter canon** [`543e07fe` — `<channel>` inside `<program>` + SSE `sleep` import; R28-C1 found already-done-S144] + **MCP-dogfood research** [MCP V0 is SHIPPED = `<program mcp>` 11-tool stdio server; the `<program mcp>` flip queued inc2; corpus-MCP own arc] + **R28-8 RATIFIED** (extend §14.10 bare-variant inference, impl-pending). **7 C1-dogfood bug-candidates filed (needs-PA-confirm)** — #6 cross-file client imports (non-module `<script>`) + #7 Tier-1 `<each>` attr/binding/handler codegen are significant. On top of v0.7.0, no version cut; full suite **22,456 pass / 0 fail / 224 skip** (862 files); known-gaps **HIGH 0 · MED 11 · LOW ~17 · Nominal 7**. PRIOR — 2026-05-31 **S150** — **source-map line-lie RESOLVED (honest-synthetic validate-at-resolution)** [`addfd205` — the B1 use-site maps recorded WRONG source byte offsets for a class of reads, so the map LIED about the line: a `@gameOver` read in a re-parsed `if=(@gameOver)` landed at byte 2 → the file's opening comment line (mario 6 hits / 3 names), and object-literal props / worker `if=` reads / reactive assigns recorded wrong ABSOLUTE offsets → mid-file wrong lines. Fix: `build-source-map.ts` now DROPS any named use-site mapping whose recorded offset resolves to a line that does not contain the identifier — a faked mapping is worse than none. Offset-source-agnostic (catches both the fragment-relative AND wrong-absolute classes); contained to the source-map-only path; no AST mutation; zero footprint when maps are off. Line-granularity by design — right-line/wrong-column marks are KEPT (devtools still jumps to the correct line); only wrong-LINE marks drop. Empirical (examples corpus): LINE-WRONG 40→0, col-drift 39 KEPT unchanged (purely subtractive), named mappings 103→63, mario 6 line-0 → 0, maps valid JSON + `node --check` clean; +2 regression tests. Root cause corrected (Rule 4): the S149 note located this at `emit-event-wiring.ts:1243` — empirically wrong (mario has no if-chains); tracing found ~40 ast-builder `safeParseExprToNode(…,0)` fragment-relative sites + a distinct wrong-absolute-offset B1 class. User-ratified "honest-synthetic now + queue full fix" — the col-precise CORRECT-provenance completion (thread real offsets) queued as a NEW LOW; the 2 S149 background-agent deaths were on this task, so PA-direct was the right call] + **S149 backlog pushed** [scrmlTS 0/3 + scrml-support 0/1 → all repos 0/0]; on top of v0.7.0, no version cut; full suite **22,450 pass / 0 fail / 220 skip** (861 files); known-gaps **HIGH 0 · MED 12 · LOW ~15 · Nominal 7**). PRIOR — 2026-05-31 **S149** — **source-map real provenance** [killed the lying `addMapping(i,0,0)` stub → real Source Map v3 with `names` field (B2) + USE-SITE spans recorded at the emit-expr choke point (B1); landed as one reviewed unit `1108d45a`; PA-verified marker-leak 0/0, use-site-not-declaration proof] + **engine `--emit-engine-graph` static sidecar** [`efae995b` — compile-time "what-comes-next" transition-graph JSON, the self-demo engine-view feed; CLI-flag arg-parse-branch fix `8765462a`] + **tier-rung re-eval DD → ratified Approach A** [`fe705c09` — SPEC §17.0.1: the existing `I-MATCH-PROMOTABLE`+`promote --match`+`W-LIFECYCLE-CANDIDATE` ARE the Tier-0→1 advisory rung; no fourth tier; S64 corpus-ouroboros reversal; witnessed-friction probe found the real friction is the Tier-2 ceiling, not the 0→1 step] + **C1 self-demo website spike** [live-edit three-column viewer — editable scrml → server-side recompile → live app + folded output + engine boxes; local-spike data for the F1 fork: live-edit is free locally, WASM only matters for a deployed static site]; on top of v0.7.0, no version cut; full suite **22,448 pass / 0 fail** (861 files); known-gaps **HIGH 0 · MED 14 · LOW 14 · Nominal 7**). Two background-agent dispatches (srcmap-attr-fix, tier-rung DD) died on a socket-death/watchdog pattern — both recovered with zero loss, DD re-run PA-direct. PRIOR — 2026-05-31 **S148** — **engine on-enter `effect=` (C1) shipped** [SPEC §51.0.H Form 3 boot-only opener `effect=` + §51.0.B/J/R/F.1 + §34 `E-ENGINE-EFFECT-ON-DERIVED` + compiler parser/SYM/codegen/tests; 3 ratified edge rulings; crash-recovered] + **match-`:>` tail cleared** [examples corpus arm sweep + standalone-`given` guard `:>` SPEC+compiler + 121 SPEC worked-example arm flips] + **self-demonstrating-website deep-dive** [crux: scrml ships a FAKED source map — `addMapping(i,0,0)` → every line maps to source 0:0; 3 forks → next-session debate; 2 experts forged]; on top of v0.7.0, no version cut; full suite **22,376 pass / 0 fail / 220 skip**, within-node 1005/0; known-gaps **HIGH 0 · MED 13 · LOW 14 · Nominal 8**). PRIOR — 2026-05-31 **S147** — **match arm-arrow `:>` canonical SHIPPED** [S145 ratification → impl: SPEC §18.2/§19/§34 + `W-MATCH-ARROW-LEGACY` info-lint (arm-context-scoped; match + `!{}` lockstep) + AST-driven `bun scrml migrate --fix` + PRIMER/kickstarter docs + 19 tests; zero codegen cost — `:>`/`=>`/`->` emit byte-identical] + **bug arc** [**R28-4** protect-analyzer now scans `?{} CREATE TABLE` in nested positions (was `children`-only) · **E-DG-002 false-positive class** closed (S146 `<match on=@cell>` + R27 C9 derived-`.filter()`-arrow) · **R28-1d** NOT-REPRODUCED · within-node-parity + LSP drift from the lint landing fixed] + **branch-leak-coherence pa.md addendum ratified**; fixes on top of v0.7.0, no version cut; full suite **22,337 pass / 0 fail / 219 skip**, within-node 1005/0; known-gaps **HIGH 0 · MED 12 · LOW 14 · Nominal 9**). PRIOR — 2026-05-30 **S146** — GITI-027B per-role SSR content-stripping **RATIFIED A+D** [SPEC §40.9.5 amended; A = server-side omission canonical-now, D = server-render-time role-gating runtime as a high-leverage arc, B rejected, C killed] + **scrml.dev-in-scrml currency revisit** [the deferred "modern site built in scrml," dormant since ~S111 — 7 commits, ~40 surgical fixes, green on v0.7.0 / 98 pages; e2e 203 pass chromium+firefox; user visually approved] + maps refreshed to v0.7.0; no version cut). PRIOR — 2026-05-30 **S145** — adopter-fix arc: **5 HIGH** [GITI-024 brace-less-`continue` parser · GITI-025+026 §37 SSE wiring · GITI-027A `W-AUTH-CONTENT-NOT-GATED` · Bug-AB-reopen engine-direct `<onTransition>`] RESOLVED + R26-verified · **SPEC §12.6** library-mode `.server.js` suppression · match-arrow **`:>` ratified** [impl carry-forward]; fixes on top of v0.7.0, no version cut; 3 parallel-load test-flakes — flake-fix in flight). PRIOR — S144 — adopter-inbox fix-wave [9 bugs] cleared + §51.0.H on-enter resolved [**Fork C1 ratified**]; fixes on top of v0.7.0, no version cut; full suite **22,263 pass / 0 real fail / 223 skip** — 3 pre-existing parallel-load flakes pass in isolation, flake-fix queued). PRIOR — **S143 — v0.7.0 cut**; gauntlet **R28** + complete fix-wave — **6 HIGH bugs + the long-deferred Bug 54 all closed; known-gaps HIGH 5→0**). Full suite **22,215 pass / 0 fail / 219 skip**, within-node parity 1005/0, TodoMVC PASS. known-gaps §0: **HIGH 0** · MED 13 · LOW 15 · Nominal 7. PRIOR — S142 (**v0.6.11 cut**; the emitted-JS parse gate flipped to a compile-time invariant **default-ON**, and **errorBoundary** built from-scratch [§19.6 + C-hybrid]; full suite 22,153/0). PRIOR — S141 (v0.6.10 cut; v0.6.8 = R27 fix-wave, v0.6.9 = parse-gate flag-gated, v0.6.10 = gate-found PARTIAL). S141 = gauntlet **R27** (Expense-Approval, 5 personas): the S140 fix-wave (Bug 57/58/59/61) HELD 5/5, and R27 surfaced a fresh batch of the same compile-clean-but-invalid-JS class — **v0.6.8** fixed C1 (two-bound `length`) / C2 (`->`-match) / C5 (`;`-in-string `!{}`) / C3 (`int`-alias), the **emitted-JS parse-gate invariant was ratified (A+D)** + dispatched as the structural class-closer, and Bug 46 (tableFor sortable/selectable) was verified-resolved. PRIOR — S140 = the **Bug-51-class corpus-coverage audit + 4-HIGH fix wave**: an 8-surface empirical audit (parallel-probe workflow + PA dual-verify) found **5 silent-miscompiles on shipped features** — clean compile + `node --check` pass but runtime-broken, all hidden behind emit-string-only tests with no happy-dom coverage. Four HIGH fixed with happy-dom acceptance gates (Bug 57 `<each>` reconcile tree-shaken-out; Bug 59 `tableFor` per-row checkbox `evt` ReferenceError; Bug 58 `formFor` validity surface never emitted; Bug 61 `@compound.<synthProp>` rollup read-path → **formFor now functional end-to-end**). Bug 54 (`tableFor :let` slot drop) + Bug 60 (render-by-tag nested compound) filed HIGH/MED + DEFERRED. The "HIGH=0" S139 milestone was partly a test-blind-spot artifact; the audit surfaced + closed the real surface. HIGH=2 (Bug 54 + the deferred surfaces) · MED=7 · LOW=12 · Nominal=7. giti + 6NZ resume-dogfooding messages sent at this cut.

### 2026-06-01 (S153 — `<each>`-in-dynamic-context fix sweep: 4 fixes + maps refresh, HIGH stays 0)

The S152 dogfood left req2 (the engine-variation todo) mounting clean but with an empty list. S153 fixed that and swept the whole surface around it — every place an `<each>` lives inside a *dynamic* mount (engine arm / match arm / nested / component body) + the `:`-shorthand parser bug the dogfood hit. Four worktree-isolated fixes + a maps refresh; req2 now renders end-to-end; HIGH stays 0; no design ratifications (the surfaced design questions were parked for user input). On top of v0.7.0, no version cut. All commits landed local-only mid-session ("commit, hold push"); the session then ended **mid-wrap on a power loss** — the 5 commits + 4 dispatch worktrees were recovered intact next session (branch-coherence `0/5`, every worktree tree byte-identical to its landed main commit, zero uncommitted work) and the wrap (incl. push) completed from that state.

- **engine-gated `<each>` never populates (`54d54d4d`, MED — the req2 blocker).** An `<each>` whose mount lives inside an engine state-child (req2's `Browsing`) never populated: at module-init the engine sits in its `initial=` arm, the each-mount is absent from the DOM, and the each-render hit `if (!_mount) return;` BEFORE reading `@cell` → `_scrml_effect_static`'s one-shot dep pass recorded no dependency → never re-fired. Three coupled codegen modes (not the hand-off's "or"): (A) `emit-each.ts` now reads `_items` BEFORE the `!_mount` early-return (always tracks the dep); (B) `emit-variant-guard.ts` + runtime — a decoupled `_scrml_each_renderers` registry + a `_scrml_remount_each(root)` helper the shared variant-swap dispatcher calls after innerHTML+wire on every arm; (C) `emit-client.ts` `detectRuntimeChunks` now descends into engine `bodyChildren` (was tree-shaking the reconcile/effect chunks out → ReferenceError). NEW 9-test happy-dom canary; full suite 22,545/0; PA-independent R26 green.
- **`<each>` in a block-form `<match>` arm emits invalid JS (`3429b385`).** Started as the S152 "#1 match-coverage follow-up test" but the empirical check found a REAL pre-existing bug: an `<each>` with the `@.` sigil inside a `<match>` arm emitted invalid JS (`E-CODEGEN-INVALID-JS`, `.name` leak). Root: match arms are raw text (`armsRaw`); `emit-match` re-parsed via `nativeParseFile` → a generic `markup tag="each"`, NOT an `each-block` (the each-block transform lives in `buildAST`, not the native parser) → rendered inline with `@.` unscoped. Fix (`emit-match.ts`): each-bearing arms re-parse via `splitBlocks`+`buildAST`, `restampEachBlockIds` namespaces the ids, lifted each-blocks attach to `matchBlock.bodyChildren` so `collectEachBlocks` emits the render fn with the `@.` rewrite; `__scrmlCachedArms` memoizes across the two passes. **Forward-correction:** the S152 #1 "covers block-form match" claim was aspirational (hook wired but each-in-match never compiled) — now real. NEW 12-test canary; full suite 22,557/0; PA R26 green.
- **`:`-shorthand child inside an engine arm breaks state-child parsing (`c89c1cb1`, parser).** A §4.14 `:`-shorthand child element (`<span : @label>`, `<li : @.name>`) inside an engine state-child broke closer-pairing → `E-ENGINE-STATE-CHILD-MISSING` (valid at top-level; only broke in an engine arm). Root: the 3 closer-finders in `engine-statechild-parser.ts` push non-void lowercase openers onto `lowerDepth`; a `:`-shorthand opener has no closer → the phantom unbalanced opener absorbed the state-child `</>`. Fix: an attr-aware `isColonShorthandOpener` (whitespace-preceded depth-0 non-string `:`; tracks string/paren/brace/bracket/`${}` so `bind:`/`on:`/`style="x:y"`/`${a?b:c}` aren't mis-detected) wired into all 3 finders, mirroring the void/self-close exclusions. The dogfood case (`<li : @.name>` in an `<each>` in an engine arm) now renders end-to-end. NEW 16-test suite + mutation probe; full suite 22,573/0; within-node 1005/0. `block-splitter.js` was already clean (the gap was confined to the custom raw-text engine parser).
- **`<each>` over an enclosing-scope binding (`e6870f25`).** Two bugs, one root (file-scope each emission can't see enclosing scope): (A) **nested `<each>`** (the documented `as` pattern) — the inner each was lifted to module-scope reading `group.items` (undefined) → ReferenceError; fix = inline emission in the outer factory via shared `emitEachReconcileLines`. (B) **`<each>` in a component body** — `@.id` E-SCOPE-001 + `.name` leak; 3 roots in `component-expander.ts` (native parser doesn't promote each/match → legacy route; `substituteProps` missed each-block string fields; tokenized `@ . id` collapse). Anchors (each in an errorBoundary / under `if=`, top-level-cell source) still work. +13 happy-dom tests; full suite **22,586 pass / 0 fail / 220 skip / 873 files** (Ran 22,807). PA R26 green (both render).
- **Maps refresh (`efcd5536`).** Incremental `project-mapper` catch-up for S148-S152 codegen. Landed mid-sweep, so the 3 later fixes above sit 3 commits past the map watermark — carry-forward refresh before the next compiler-source dispatch.
- **NEW carry-forward / surfaced this session.** **#2f — the native parser does NOT promote `<each>`/`<match>` to structural each-block/match-block nodes** — a hard M5-swap precondition, now witnessed TWICE (the each-in-match fix + the each-in-component fix both route AROUND it via the legacy BS+TAB path; when the native parser becomes default, it MUST promote `<each>`/`<match>` or every each/match breaks). **Design questions PARKED for user:** **`:`-shorthand on a lowercase HTML element** (`<span : @label>` parses but emits empty `<span></span>` + E-DG-002 even at top-level — should it render? §4.14 line 997 makes it a design call); **SPEC §4.14 vs §51.0.I `:`-placement discrepancy** (§4.14 `<tag : expr>` vs §51.0.I `<Variant> : expr` for the state-child's own body — authority call); **scalar/struct zero-default** (`<x>: int`→`0`? `<x>: string`→`""`? — Shape 4 did array-only). Plus the carried S151/S152 set (C1 inc2, conditional-tier CPS debt, A-4 atom-emitter, predicate-fields question, Tier-2 ceiling DD).

### 2026-06-01 (S152 — dogfood-driven fix chain: 5 fixes + Shape 4, HIGH→0)

The entire session was **dogfooding the language by writing a real app** — `masterScrml/req.scrml` + `req2.scrml`, the scrml rendering of TJ DeVries' CLI-todo language (first-class `database`, Ecto queries, `provide` capability injection, tagged-error `match Int.parse`, `match Runtime.args` dispatch). Writing it (and an engine variation) surfaced and closed a chain of gaps — the C1-self-demo thesis ("real usage surfaces real gaps") in action. Five worktree-isolated fixes shipped + one grammar feature; HIGH back to 0; one deep-dive; three design ratifications. On top of v0.7.0, no version cut.

- **#7 — `<each>` body interactivity (`9c192c73`).** Tier-1 `<each>` per-item elements silently dropped event handlers + `class:` bindings (inert `setAttribute`) and `@.` in an attribute value was a hard `E-SCOPE-001`. Fix: `type-system.ts` `inEachBodyScope()` (the `@.` sigil resolves in attr-value position inside an `<each>` body, SPEC §17.7.3) + `emit-each.ts` real per-item codegen (`class:`→`classList.toggle`, handlers→`addEventListener`, `${}`/`@.x`→`setAttribute(String(...))`). +18 tests (8 emit + 10 happy-dom).
- **scrml dev fixes (`efc23ecf`).** `scrml dev` from a large dir crashed with `ENOSPC` (the file-watcher was recursively watching the input's whole parent tree incl. `node_modules` + sibling repos → blew `fs.inotify.max_user_watches` → uncaught throw → server died) AND served a stale entry. Fix: per-file watch over the bounded gathered `.scrml` set (never touches node_modules) + `error`-handler graceful degrade + root-`/` entry-preference (`<input>.html` over the first-`.html`-in-dist fallback). **This + #6 were the actual "nothing renders" the user hit at session start.**
- **#6 — cross-file CLIENT module-loading (`5082ff3c`, Approach B).** Multi-file apps emitted a raw ES `import { x } from "./dep.client.js"` (and module-only `export`) into a file loaded as a CLASSIC `<script>` → "Cannot use import statement outside a module" → the whole client.js failed to parse → no client code ran. Per the deep-dive (`scrml-support/docs/deep-dives/client-cross-file-module-loading-2026-06-01.md`; verdict B, A-4-reuse ruled out): a `_scrml_modules` registry (mirrors `_scrml_stdlib`) — exporter footer + importer registry-read + topo-ordered dependency `<script>` emission + IIFE-wrap of cross-file-linked bodies (a collision the DD's example would have hit). +7 tests incl. a happy-dom multi-file test (closes the corpus-coverage gap). The bug was invisible to `node --check` (Node auto-detects ESM), the test suite (no browser test loaded multi-file client.js), and VERIFIED.md.
- **inline `?{}` in a conditional branch (`893872e3`).** Inline `?{}` SQL inside a `match`-arm body / `if` branch wasn't recognized as a server-call boundary by the body-split/CPS classifier → raw `?{}` emitted (`E-CODEGEN-INVALID-JS`) or whole-fn server-escalation (`E-RI-002`). Fix: `isServerTriggerStatement`/`analyzeCPSEligibility` recurse into control-flow bodies. Coupled fix (Rule 3): the match-stmt server-emit used a sync IIFE (broke nested `await`) + read `@cell` instead of the marshalled `_scrml_body`, AND **leaked `_scrml_sql` into client.js** (the `E-CG-006` security-net catch) — now async-IIFE-wrapped + payload reads; zero SQL in client. +12 tests.
- **SPEC §6.2 Shape 4 — typed-array no-RHS → `[]` (`46f9bb55`).** `<x>: T[]` may omit the initializer and defaults to `[]` (a DEFINED value per §42.1.1, `is some` not `not`); a non-array typed declaration with no RHS → new `E-DECL-NEEDS-INITIALIZER` (closes the silent-undefined hole). Root was front-end (the no-RHS decl fell through to an html-fragment, never recognized as a state-decl). +15 tests. (Scalar/struct zero-defaults left OUT — separate open Q.) User-ratified off the dogfood.
- **each-render-before-cell-init crash (`25a1c243`, HIGH).** The auto-generated `<each>` render fn was CALLED at module-init BEFORE its source cell's `_scrml_reactive_set` → first render hit an uninitialized cell → `_scrml_reconcile_list(undefined)` crash. Broad: any no-`<empty>` `<each>` over a same-program cell (compile-clean, runtime-dead). Fix: defer the each-dispatchers until after the cell-init `reactiveLines` (ordering) + `!_items`/`Array.isArray` guards. The #7-test blind spot: every existing each-test carried an `<empty>` block whose `!_items` guard masked it; unit each-tests are emit-string-only. NEW happy-dom test loads the emitted client.js in real module-init order. +90 tests net; browser tier 294/0.
- **Design ratifications:** the **`---` answer-delimiter convention** (tail below the last `---` = answers to PA's pending questions) → pa.md. **DQ-2 conceded** — array mutation methods (`push`/`pop`/`shift`) do NOT trigger reactivity (§6.5 DQ-2); reassignment (`@arr = [...]`) is canonical; the user conceded the Vue/proxy reactive-array direction (no DQ-2 reopen). **Shape 4** (above).
- **NEW open (carry-forward):** **engine-gated-`<each>`-populate** (MED — an `<each>` whose mount is inside an engine state-child skips dep-tracking at module-init [hits `!_mount` before reading the cell] → never re-fires; req2 mounts clean but the list stays empty; the immediate req2 blocker); the **A-4 atom-emitter** bare-import follow-up (same class as #6, gated on `emitPerRoute`); the **scalar/struct zero-default** open Q; the **conditional-tier (A9 Ext 3) / multi-batch-across-a-branch** CPS debt (server-in-arm + server-after-match, and `!{}`+server in one arm body, still need extraction to named fns — the body-split's remaining envelope, now well-characterized by the dogfood); the **W-DEAD-FUNCTION** (match-arm-only-called fns) + **I-FN-PROMOTABLE** (SQL fns) cosmetic lint false-positives.

### 2026-06-01 (S151 — C1 self-demo website inc1 + C4 lifecycle fix + R28-C2 canon + MCP-dogfood research)

The S148-ratified self-demo website went from a lost `/tmp` spike to a real, landed, byte-verified increment-1; plus a MED safety-gap fix, a canon-fix, and the grounded MCP-dogfood answer. On top of v0.7.0, no version cut. Three BG workflows ran (C1 build, C4 fix, MCP research) while the foreground drove canon-fixes; everything staged-not-pushed until "wrap and push". (Session note: a prior "S151" was a void start — reclaimed; S150 CLOSE preserved at `handOffs/hand-off-155.md`.)

- **C1 self-demo website — increment 1 (`c66af6b2`).** The compile-transparent viewer, built AS a scrml app, in `docs/website-viewer/` (21 files). Layout (S151 revision): site-left-60% (live `14-mario-state-machine` iframe) + right-40% STACKED — scrml source / engine "what-comes-next" diagram (engine-conditional) / tabbed JS·HTML·CSS output; provenance links vertically. **The provenance is REAL** — the committed `.js.map` + `.client.js` + engine-graph are byte-identical to an independent regeneration from the canonical source; verify VLQ-decoded the map itself (hover src-line 69 `@coins=@coins+n` → JS line 52, bidirectional, line-honest, synthetic lines excluded). NOT a 0:0 stub — the credibility thesis holds. Engine box reads real `--emit-engine-graph` JSON (mario's MarioState fan-out + derived HealthRisk). Plain-HTML nav + dashboard stub; reproducible precompute (`build-artifacts.mjs`) + `serve.sh`. **Two deviations:** path is `docs/website-viewer/` (sibling, NOT nested — a nested viewer regresses the 97-page-site compile); serve via `serve.sh` (symlinks `/data`; bare `scrml dev` 404s the artifacts). **Serve-before-push (S146) overridden** by explicit user "commit C1" on the verification (build-agent Puppeteer-verified); `serve.sh` available to eyeball live. Decisions ratified: showcase-only (live-edit→C2a) · shell-as-scrml-app · viewer+flagships+dashboard-then-iterate. inc2 = 3 more flagships + live dashboard + KB-nav + PE-layer toggle + postMessage live-pane↔source hover + Phase-2 HTML/CSS provenance.
- **C4 / R28-5 — object-literal lifecycle E-TYPE-001 dormancy RESOLVED (`cce289b4`).** Function-local `const u: User = {…}` object-literal bindings skipped the lifecycle tracker (pre-transition field read compiled clean = silent safety gap). Root: `collectStructBindings` (type-system.ts) had JSX + positional-tuple enrollment but no `{`-object-literal branch (unlike the working sibling `collectStateDeclStructBindings`). Fix (+22/-1): Path 4 reusing the existing `seedInitialFromObjectLiteral` seeder; enrollment-only (walker untouched), gated on `lifecycleRegistry` (no over-fire), carve-outs preserved. +10 tests; reproduce→fix→verify BG workflow, independently verified (E-TYPE-001 fires pre / clean post; JSX 6/6, Shape-1 87/87, carve-out 2/2). **NEW LOW filed:** the struct-field walker doesn't honor `given (u.field is not not)` discrimination — PRE-EXISTING (JSX form identical), not introduced by C4.
- **R28-C2 — kickstarter canon-fix (`543e07fe`).** §11.3 real-time recipe: `<channel>` was a sibling of `<program>` (fires E-CHANNEL-OUTSIDE-PROGRAM per SPEC §38.1 / Insight 30) → moved inside `<program>`; PA compile-verified the fixed recipe exit-0 clean. §11.13 SSE: added `import { sleep } from 'scrml:time'`. **R28-C1 found STALE-open** (the `server fn`→`server function` flagship fix already landed S144 `44d61a19`; reverse-direction caught it — no action). Parked: `print()` (~15 sites, not a defined builtin — canon-wide decision) + `< db>` markdown-spacing.
...
- **Source-map real provenance (B2+B1, `1108d45a`).** Killed the degenerate `addMapping(i,0,0)` stub (every output line → source 0:0, a structurally-valid map that LIES). B2 (`source-map.ts` rewrite) added real generated+source columns, the `names` field (§47 encoded `_scrml_t_count` → author `count`), `x_scrml_kinds` source/synthetic sidecar, `sourcesContent` privacy-default-OFF. B1 replaced B2's declaration-footprint resolver (every use mapped to its *declaration* — the deep-dive's eliminated post-hoc approach) with USE-SITE spans recorded via gated inline sentinels at the `emit-expr` reactive get/set/postfix choke point. PA-independent R26: marker-leak 0/0 on shipped JS, `node --check` clean, counter `@count = @count + 1` (use, src line 4) maps to line 4 NOT the `<count>` decl (line 1). Fulfills the existing SPEC §47 source-map promise; no new normative text. Landed via S67 file-delta SOURCE-MAP-FILES-ONLY (B1's branch predated engine-graph; full diff would have reverted it — caught at landing).
- **Engine `--emit-engine-graph` static sidecar (`efae995b` + flag-fix `8765462a`).** Compile-time projection of each engine's transition graph → `<base>.engine-graph.json` (varName/forType/initialState/variants + flat transitions[wildcard-marked] + per-state next[wildcard-expanded] + lifecycle/effect flags + derived flag). Mirrors `--emit-reachability`; pure data-exposure, no runtime change, `codegen/index.ts` untouched. The C1-static "what-comes-next" feed; serves the C2a playground too. Honest-empty `{"engines":[]}` for engineless files; byte-deterministic. The CLI flag shipped without an arg-parse branch (declared+consumed+advertised-in-help but `--emit-engine-graph` threw "Unknown option") — fixed `8765462a`; root-cause was R26 exercising the API path not the CLI surface.
- **Tier-ladder intermediate-rung DD → ratified Approach A (`fe705c09`).** Re-evaluated the S64 debate-04 rejection (corpus-zero = the ouroboros Rule 2 forbids). Empirical witnessed-friction probe (82 adopter files): the 3-rung ladder is actively used (232 `if=` Tier-0 + 74 structured `<match>`/`<engine>`), and the genuine friction is the **Tier-2 ceiling** (missing event-payload-transition primitive — triage-board `dragPhase` declared the enum, built the engine, still dropped to a Tier-0 `if`-guard) + lifecycle `transition()` (§14.12.6), NOT the Tier 0→1 step. Approach A: the existing `I-MATCH-PROMOTABLE` + `promote --match` + `W-LIFECYCLE-CANDIDATE` ARE the advisory rung; no fourth tier (NEW SPEC §17.0.1; SPEC-INDEX regen). DD authored PA-direct after the `scrml-deep-dive` agent stalled twice. Rule-4 note: §17.0.1 did not previously exist — the DD's "flagged for re-evaluation" claim was a brief-inherited miss, corrected; §17.0.1 created as the disposition. DD `scrml-support/docs/deep-dives/tier-ladder-intermediate-rung-2026-05-31.md` (status: historical).
- **C1 self-demo website spike (local, not committed).** Live-edit three-column viewer: editable scrml source → debounced server-side `compileScrml` recompile → live running app (fixed-height pane) + compiled output (synthetic runtime lines folded) + engine "what-comes-next" boxes, hover-linked via the real `.js.map`. Proves the C2a live-edit loop is achievable locally with zero WASM cost (real F1-fork data: WASM only matters for a deployed *static* site). Spike lives in `/tmp`; building it into `docs/website/` is the next C1 increment.
- **CARRY-FORWARD (located, not fixed):** the source-map use-site work has a **6-hit / 3-name residual** (gameOver×2, healthRisk×3, lives×1) where `if=`/`show=` markup-attribute-expr + interpolation reads record FRAGMENT-RELATIVE span offsets (`@gameOver`→`start=1`) instead of absolute file offsets, mapping to comment line 0. Root cause LOCATED: `emit-event-wiring.ts:1243` passes base-offset `0` to `parseExprToNode`. Fix = thread the absolute base offset. Two background agents tasked with this died on the socket-death pattern; recommend PA-direct next.

Three arcs, all pushed (scrmlTS `09f74bee → a0f61a20`, 7 commits; scrml-support 2 commits). Fixes/features on top of v0.7.0, no version cut.

- **Engine on-enter — `effect=` on the `<engine>` opener (Insight 33 Fork C1).** SPEC §51.0.H NEW Form 3 (boot-only init effect — fires once at module-init = the implicit init→`initial=` edge; Elm `init`+`Cmd`) + §51.0.B/J/R/F.1 + §34 `E-ENGINE-EFFECT-ON-DERIVED` + README flagship fix + PRIMER §7 (`8056ff5d`); compiler parser + SYM + codegen + tests (`e41c95d4`). 3 user-ratified edge rulings: errorBoundary render-only (boot effect routes `!` via its own `!{}`; non-`!`→§19.6.8 backstop) · onIdle arm-then-fire · derived-opener forbid. Write-validation deferred (B15 raw-text precedent; `.skip` test). PA-independent R26 green. **Crash-recovered** — first compiler dispatch socket-crashed mid-codegen; checkpointed + finish-dispatch inherited via merge; zero leaks.
- **Match-`:>` tail cleared.** Corpus arm-arrow sweep (`07bc712c` — 32 `=>`/`->`→`:>` across 7 examples, arm-arrows only) · standalone-`given` guard `:>` — SPEC §42.2.3 + §34 `W-GIVEN-ARROW-LEGACY` (`5b24c46f`) + compiler (`a0f61a20` — `separatorGlyph`, lint with verified no-double-fire, `rewriteGivenGuardArrows` migrate rule, byte-identical emit) · SPEC worked-examples (`8d2d699b` — 121 match/`!{}` arm flips, lint-oracle-driven, cherry-picked clean). Decisions via AskUserQuestion (standalone-given flips; corpus sweep now). **3 follow-ups filed:** source-map stub (below) · `derived=match` arms not covered by the match-`:>` lint/migrate · `migrate` rewriting `<machine>` inside comments.
- **Self-demonstrating compile-transparent scrml.dev — deep-dive done + pushed** (scrml-support `23433ea`). User thesis: "no one takes scrml seriously until they see it really in action." Vision: self-demo (the site IS a scrml app showing its own real source↔output↔live), engine-heavy, real provenance, full replacement, next-gen nav, PE-layer range toggle (no-JS↔SPA), auth dogfood (sign-in/prefs/settings), and a **playground** (= the in-browser-compile fork made a destination — likely the highest-leverage adoption artifact). **Crux finding (source-verified):** scrml's source maps are a degenerate stub — `codegen/index.ts addMapping(i,0,0)` maps every output line to source 0:0 (structurally-valid Source Map v3, zero provenance; §47.5 promises real maps). 3 forks → next-session debate; 2 experts forged. User greenlit (c) no-regrets moves (engine "what-comes-next" view + fix the source-map stub); debate first thing next session.

### 2026-05-31 (S147 — match arm-arrow `:>` canonical SHIPPED + bug arc [3 closed / 1 not-reproduced])

**match arm-arrow `:>` canonical — ratified S145, shipped end-to-end S147.** `:>` is now the canonical match + `!{}`-handler arm separator; `=>` and `->` are deprecated aliases. All three parse, build, and emit **byte-identical** JS — this is a canonical-flip + enforcement, not a behavioral change (zero codegen cost, verified).
- **SPEC §18.2/§19/§34** (`a2930106`): grammar production adds `:>` (was undocumented despite being corpus-dominant); `:>` declared canonical, `=>`/`->` deprecated; §18+§19 alias-note callouts; NEW `W-MATCH-ARROW-LEGACY` §34 row (info-level, arm-context-scoped); `E-MATCH-ARM-SEPARATOR` text refreshed; SPEC-INDEX regenerated. Wildcard `else`/`_` + variant `.`/`::` UNCHANGED.
- **Compiler enforcement** (`f444290a`, dispatched): `ast-builder.js` `matchArrowGlyphAt` helper + glyph-preservation on all match-arm + `!{}`-handler-arm construction sites (the `->` arm now builds STRUCTURED nodes — was `bare-expr`; `->` deliberately NOT merged into one OPERATOR, protecting the `fn … -> Type` return path); `type-system.ts` `W-MATCH-ARROW-LEGACY` emission (match `checkMatchDiagnostics` + `!{}` `guarded-expr`, lockstep, info-severity → `result.warnings`); `migrate.js` AST-driven `rewriteMatchArmArrows` + `--fix` (rewrites ONLY arm-separator arrows — arrow-functions + fn-returns untouched by construction); 19 tests. PA-independent R26 verified byte-identical pre/post-migrate emit.
- **Docs** (`f0d7db3b`): PRIMER §6/§6.2/§7 + kickstarter match/derived-match/handler/anti-pattern arm separators → `:>` (clean cases). **Within-node parity rebump** (`1d698cbb`, S125 companion): the live-side `armArrow` field is a benign live-ahead divergence from the native parser → MISSING-FIELD allowlist rebump on the 5 `!{}`-handler error-fixtures. **LSP outline filter** (`cef5ed98`): `buildDocumentSymbols` tolerates the advisory `W-MATCH-ARROW-LEGACY` on the Mario fixture's `=>` arms (same class as the 3 advisory lints it already filters).
- **Deferred (deprecation-window tail):** corpus mass-migration (`migrate --fix` ready, but bundles the `<machine>`→`<engine>` baseline migration — decide first), SPEC worked-examples (manual), standalone-`given x => body` scope-question (deep-dive scoped match+`!{}` only).

**Bug arc (HIGH=0; "move to bugs"):**
- **R28-4 RESOLVED** (`bf5ad0db`) — `extractCreateTableStatements` (protect-analyzer.ts) recursed ONLY `node.children`, so a `?{} CREATE TABLE` under `body` (top-level `${}` logic block + fn-decl bodies) was invisible → spurious `E-PA-002` while the message told adopters to add exactly that. Fix = generic cycle-safe deep-walk. Message was correct; scanner was broken. +3 regression tests (43→46/0). PA R26: both reproducers build the shadow DB; genuine-missing guard holds.
- **E-DG-002 false-positive class RESOLVED** (`07655674`, dispatched) — `E-DG-002` ("declared but never consumed") fired spuriously on (A) a `@var` read inside a derived `.filter()` arrow body (R27 C9) and (B) a block-form `<match on=@cell>` header (S146). Pure reader-accounting under-counts; codegen + runtime were already correct. Fix: DG-local `collectLambdaBodyReactiveRefs` (does NOT widen the shared `forEachIdentInExprNode` — preserves lin-capture) + `match-block` `onExprRaw`/`armsRaw` raw-credit. Guard: genuine-unused still fires. +8 tests.
- **R28-1d NOT-REPRODUCED** — canonical bare-`<program>` + `<each>` emits each-wiring on HEAD; closed (re-open with the exact R28-1b dev source if it resurfaces).

**Process:** branch-leak coherence addendum ratified into pa.md (S147) — verify `git rev-list --left-right origin/main…HEAD` divergence + branch-tip==FINAL_SHA on every dispatch landing, not just `git status` (closes the S142 committed-leak-onto-local-ref class). The within-node-parity + LSP failures were the lint landing's two **pre-commit-excluded** consequences (S78/S125) — surfaced only by the full-suite gate; neither was a behavioral regression. **NEW LOW (deferred):** `<` inside a markup-region lambda body parse-truncates → `E-DG-002` false-fires on the post-`<` cell as a symptom (tokenizer `<`-disambiguation). **Tiny follow-up:** SPEC §34 E-PA-002 row summary is stale. known-gaps: MED 14→12 · LOW 15→14. Full suite **22,337 pass / 0 fail**.

### 2026-05-30 (S146 — GITI-027B per-role SSR ratified A+D + scrml.dev-in-scrml currency revisit)

Session-start map refresh (project-mapper cold-start → v0.7.0 watermark `948d3f2f`, 38 commits stale; `46229a39`). Two arcs, no version cut.

**GITI-027B — per-role SSR content-stripping ratified A+D** (deep-dive `scrml-support/docs/deep-dives/giti-027b-per-role-ssr-content-stripping-2026-05-30.md`; design-insight 35; `4541d8a0`). SPEC §40.9.5 was already normative that `<auth role>` is a JS-mount/code-split gate (NOT content-secrecy), with per-role HTML stripping consciously deferred — so 027B = lift-the-deferral, not a bug (027A's `W-AUTH-CONTENT-NOT-GATED` warning stands). User ratified directly (no debate run): **A** (server-side omission — sensitive content/data/mutations behind a `server function` that role-checks server-side) canonical-now + PA-recipe-verified (giti's secret emits to `.server.js` only); **D** (server-render-time role-gating runtime) ratified as a high-leverage-gated arc (spec-ahead-of-impl, Nominal); **B** (per-role static HTML, = OQ-A4-E option (b) S91-rejected) rejected; **C** (runtime DOM-prune) killed (security theater). giti unblocked via the A recipe (reply sent); pushed.

**scrml.dev-in-scrml — full currency revisit** (the deferred "modern site built in scrml," dormant since ~S111). 7 commits, ~40 surgical fixes, site green on v0.7.0 (98 pages):
- **Phase 1** — cleared 8 bare-`/` `E-SYNTAX-050` (standalone `/` separators in prose → `&#47;`).
- **Phase 3 currency** — **auth.scrml security-footgun reframe** (was teaching `<auth role>` as content-security enforcement — the exact 027B misconception) + new `W-AUTH-CONTENT-NOT-GATED` page; **5 leaked "agent-only" scratch-work blocks removed** (rendered `/home/bryan/...` paths + private bookkeeping; the marker-audit found 4, PA caught a 5th in server-boundary-disappears); **2 UNBUILT primitives corrected** (`invalidate()`, `scrml migrate` DB-apply → dev-reload schema-differ); **v0.3 placement reversals** (channels + schema → inside `<program>`); sql `runs:`/`returns:` fabricated server-fn syntax → brace bodies; §11→§52 citations; status-note + landing reframes (block-match + engine-codegen shipped); **version policy** (site-wide stamp → v0.7.0, 15 per-feature "Available since" chips dropped); **reference-medium** (lift post-S132, engine `@light` casing, match lint severity, validators ×3, req schema, onIdle broken self-write example, reference/index bogus `derived` keyword); **SPEC-INDEX §38** stale channel-placement row.
- **Phase 2 visit-verify** — Playwright e2e **203 passed (chromium + firefox)**: HTTP 200 + shell composition + no console errors + link-integrity across all pages; user visually approved at `localhost:3000`. (webkit env-blocked — missing `libwoff2dec.so.1.0.2` system lib, not a site defect.)
- **Deferred (cosmetic, user's call):** 12 availability-table "Since" columns + bare-prose engine/match state-child bodies (compile today, non-canonical). **NEW LOW candidate (needs-confirm):** block-form `<match on=@cell>` doesn't register `@cell` as a DG consumer → spurious `E-DG-002`.

### 2026-05-30 (S145 — adopter-fix arc [5 HIGH] + §12.6 library-mode `.server.js` suppression + match-arrow `:>` ratified)

Second adopter-fix arc in two days. Triaged a fresh inbox (3 giti + 1 6nz reopen), R26-verified every report on current HEAD before acting, and landed five fixes — each PA-independently re-verified on its landing commit (S138 doctrine). Plus a design deliberation that ratified `:>` as the canonical match arm-arrow.

- **GITI-024** (`8b50c89b`) — a brace-less `if (cond) continue`/`break` whose following statement is identifier-led no longer swallows that identifier as a labeled-continue target (`continue out; . push(line)` → invalid JS, caught by `--validate-emit`). Root was a parse-time bug in `ast-builder.js parseLogicBody`: the label heuristic compared `tok.line` (which doesn't exist — line lives at `tok.span.line`), so the guard was always true and any next-line identifier was consumed as a label. Fix mirrors the already-correct `return`-stmt heuristic; 4 sites. +5 tests.
- **§12.6 library-mode emission** (`3b825808`) — in `--mode library`, a function escalated to server-bound purely by body content (server-only import / `?{}`) now emits as a plain server-side export; the §12.3 HTTP route-handler / `.server.js` bundle is suppressed (an explicit `server function`/`route=` retains it for `mount(server)`). NEW SPEC §12.6 + §21.5 cross-ref, grounded in §12.3 / §13.4 / §21.5 / §44.7.1. Resolves the GITI-027 secondary (spurious dead `.server.js`) and makes the GITI-024 symptom unreachable in library mode. +4 tests.
- **Bug-AB reopen** (`2ebd107a`) — the canonical engine-direct `<onTransition from=.X to=.Y>` (a direct child of `<engine>`, per SPEC §51.0.H / PRIMER §7) was dropped at parse time (the engine-statechild scanner only accepted uppercase-led openers), so its effect never fired. S144's `5113f3ea` had fixed only the write-routing half; the record is corrected. Fix: `scanForEngineDirectOnTransitions` over the full engine body + an `engineOnTransitions` meta field + a `collectEngineHooks` "direct" arm; native-walker mirrored; NO runtime change. happy-dom: `@transitions` increments 0→1→2. +4 tests.
- **GITI-025 + 026** (`e2dcde7b`) — §37 SSE client-stub wiring. A parameterized `server function*` now binds its params from `route.query` server-side and encodes them into the EventSource URL client-side (was an empty stream); `@cell = gen()` binds a per-event callback instead of storing the EventSource object; named-event generators register `addEventListener`. Runtime drain: `countdown(5)` yields 6 frames (was 0). +15 tests.
- **GITI-027A** (`53203851`) — NEW `W-AUTH-CONTENT-NOT-GATED` warning: `<auth role>` gates JS mount/behavior only (and only under `--emit-per-route`), NOT served HTML content — surfaces a security footgun that was previously silent (the gated markup ships verbatim to all viewers). SPEC §34 + §40.9.5. Part B (per-role SSR content-stripping) deferred to a design deliberation. +11 tests.

**Match arm-arrow `:>` ratified** (design-insight 34; deep-dive `scrml-support/docs/deep-dives/match-arrow-colon-canonical-2026-05-30.md`): `:>` becomes the canonical match/handler arm separator; `=>`/`->` become deprecated aliases (`W-MATCH-ARROW-LEGACY`, lint→error window). `:>` was already the corpus-dominant arrow (yet undocumented in SPEC), already codegens byte-identically to `=>`, and was the author's original S14 intent (it differentiates from the JS arrow-function `=>` and is easier to type). Wildcard `else`/variant `.`/`::` left as-is. Implementation is a carry-forward arc — zero behavioral risk (all three arrows emit identically; AST-driven `bun scrml migrate --fix` + docs migration; `!{}` arms move in lockstep).

In flight at close: a test-infra fix making 3 parallel-load-flaky tests (`self-compilation` ts/ast parity + `trucking-dispatch` two-compile determinism) parallel-safe, so the pre-push full-suite gate stops forcing `--no-verify` pushes.

### 2026-05-30 (S144 — adopter inbox fix-wave [9 bugs] + §51.0.H on-enter resolved [Fork C1 ratified])

Cleared the full adopter inbox — 6 reports (4 giti + 2 6nz), 9 genuine bugs — as 6 file-delta landings, all R26-verified + gated, **zero path-discipline incidents**. No version cut (fixes on top of v0.7.0).
- **A+B** (`8e7f18fe`) — GITI-020/021/022: server-function nested-block emission now threads per-fn context, so a channel-cell `@cell=` write nested in `if`/`for` reaches the §38.4 broadcast lowering (was an undefined client `_scrml_reactive_set`) and a bare reassignment no longer emits a shadowing `const` (the S34 client `declaredNames` fix, mirrored into the server-fn body path).
- **F** (`c6cd6538`) — Bug AC: §36 input-state `<#id>` reads resolve through `_scrml_input_state_registry.get(...)` instead of an unbound `_scrml_input_<id>_` (the whole keyboard/mouse surface was runtime-dead). +happy-dom gate.
- **C-Z** (`88071273`) — Bug Z: the post-emit fn-name mangler is fenced through the string/regex/comment-aware `rewriteCodeSegments` — a declared name inside a string literal is no longer rewritten (critical for an editor rendering code-as-text).
- **C-X** (`e50ee9c2`) — Bug X: `//` inside a string literal (incl. `https://`) no longer breaks block-splitting; a line-scoped, regex-tolerant `//`-gate (the naive full string-skip was reverted — it broke self-host bootstrap on regex-`"`).
- **D** (`93d8cab4`) — Bug Y + Bug AA: 2 NEW §34 diagnostics — `E-MATCH-ARM-SEPARATOR` (typer; comma-separated match arms, both markup + `let/const = match` forms) and `W-MATCH-VALUE-UNUSED` (a plain `function`'s discarded bare-tail `match`; per §48.11 plain functions have no implicit return — diagnostic, not a semantics change).
- **E / Bug-AB** (`5113f3ea`) — 6nz Q-AB (a real codegen bug, not a question): `<onTransition>` now fires from a program-scope `function` trigger (engine context threaded into free-function bodies so `@var=.X` / `.advance()` route through the dispatcher + `fire_hooks`), and `@engineVar=.X` inside an `<onTransition>` body no longer trips a phantom `E-ENGINE-VAR-DUPLICATE`/`E-DG-002` (`inEngineBody` walk-guard in symbol-table). Same context-threading root family as A+B.

**GITI-023** (`?.` → `? .`) was **NOT reproduced** on v0.7.0 (already fixed v0.6.7→v0.7.0) → closed as resolved.

The Q-AB landing surfaced + resolved a real spec gap (`<onTransition to=.SameState>` self-target is `E-ENGINE-INVALID-TRANSITION`; there was no clean on-enter / on-initial-mount form — the README Stage-3 flagship reaches for the invalid form). Ran a deep-dive → debate → **ratified Fork C1** (`scrml-support` design-insight 33): `effect=` gains a second legal host — the `<engine>` opener — meaning the `init→initial=` edge's effect, fired once on boot. Sub-shape "on-enter-from-any-source" ruled not-real-for-v0.x (every in-edge is a named, statically-enumerable `rule=` member); `<onEnter>` held pre-priced as the reopen-trigger. Implementation (SPEC §51.0.H amendment + `effect=`-on-opener + codegen + README flagship fix) pending. Process notes: a workflow debate-judge stalled on API overload → recovered via S83 (salvaged the cached positions, ran a fresh judge); a judge `design-insights.md` overwrite was caught at commit-verify + recovered from git. Pushed with **user-authorized `--no-verify`** (3 pre-existing parallel-load flakes block the pre-push; all pass in isolation; flake-fix queued for next session). Full suite **22,263 / 0 real fail / 223 skip**.

### 2026-05-30 (S143 — gauntlet R28 + complete fix-wave [6 HIGH + Bug 54 closed; HIGH→0] + v0.7.0 cut)

Ran **gauntlet R28** (Content-Publishing-Platform "Press", 5 personas React/Go/Elixir/Svelte/Pascal) as two background workflows — build (dev-returns-content) → 5-overseer verification + synthesis — with PA dual-verify (R26) before any bug was filed. The round **validated the two S142 investments**: errorBoundary (§19.6) works end-to-end across all 5 (zero walls — elixir: *"worked exactly as §19.6 documents on first read"*); the parse gate (default-ON) had **zero false positives** across 5 clean compiles and caught **2 real codegen-defect classes**. The overseer caught 2 dev misreports (react + svelte on the variant-progression `transition()` path). Reports: `scrml-support/docs/gauntlets/gauntlet-r28-report.md` + `gauntlet-r28/OVERSEER-REPORT.md`.

**Then the fix-wave closed everything — 6 HIGH + the long-deferred Bug 54 (known-gaps HIGH 5→0), all PA R26-verified:**
- **R28-1** (`e6fb2f3d`) — `@.`-leak in `<match on=@.>` inside `<each as>` (gate-caught); `emit-match` now lowers `on=@.field` → the iteration variable.
- **R28-3** (`051ce984`) — a `//` markup comment before a `:`-shorthand `<engine>` state-child broke block-splitting (the compound-auto-lift scanner landed on `/`); fixed with `skipTriviaForCompoundScan` (SPEC §27.1 — `//` is universal trivia).
- **R28-6** (`0ecfab98`) — variant-progression `transition()` enforcement was dormant; the dispatch agent **empirically corrected the brief's root-cause hypothesis** (the `.get()` loose-return was a red herring — the gap was the type-system `state-decl` handler skipping RHS read-scans on reactive assignments). Symmetrically closed the same dormancy for presence-progression E-TYPE-001.
- **R28-2** (`0dbef110`) — tableFor `<column>` row-access was broken BOTH ways (`:let` forwarded-as-HTML; `@row` → `_scrml_reactive_get("row")`); fixed via §16.6 arrow re-parse + `@row`→loop-local. **The long-deferred Bug 54 is un-deferred + CLOSED.**
- **R28-7** (`4144dc30`) — schemaFor + tableFor now map `T | not` (nullable) + `T?` optional fields (5/5 R28 devs hit the rejection). SPEC §41.15.8a (→ nullable column = base type minus `req`/`NOT NULL`; the round-trip inverse of §14.8.3) + §41.16.6a (→ value-or-empty-`<td>`, guarded so never literal `"null"`/`"undefined"` — consistent with the S89 `""`-is-defined axiom). Exactly-`[T,not]` qual; nullable enum; `req`+`|not` conflict → nullable wins.
- **R28-1b** (`1d227a74`) — block-form `<match>` inside `<each>` wasn't rendered per-item (the each factory dropped the `match-block` child + a module-scope dispatcher referenced the item var out of scope). Now emits the match per-item inside the each factory (item-scoped mount + per-mount dispose isolation). happy-dom verified (each `<li>` renders its own arm).
- **R28-C1** (`44d61a19`) — canon-fix: SPEC §14.12.6.2 + PRIMER §6.5 flagship variant-progression worked examples used `server fn …?{}` (which fail E-FN-001, since `fn` is pure); corrected to `server function`. 4/5 devs hit it verbatim.

**Cut v0.7.0** (`c5dbf15d` + tag; S94 bump-on-tag): the errorBoundary build (S142) + the R28 fix-wave + the `T | not` L22 nullable mapping. **Full suite 22,215 / 0 / 219**, within-node 1005/0, TodoMVC PASS. Process: 6 worktree dispatches all clean-landed (R28-2/R28-6 collided on `type-system.ts` → 3-way patch with disjoint regions; R28-7/R28-1b ran in parallel scope-fenced). **NEW findings filed for S144:** R28-1c (`<each>` same-key per-item-reactivity gap), R28-1d (bare-`<program>` default-logic drops `<each>`), R28-2b (tokenizer leading-`:` strip), R28-7b (predicated-base-in-union), R28-8 (bare-variant inference into object-literal fields — design call), R28-4 (E-PA-002 misleading diagnostic).
