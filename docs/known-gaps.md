...
| **R28-1b** | HIGH | OPEN — **CONFIRMED S143 (PA empirical, `/tmp/r28-1b/`); fix-dispatch in flight** | block-form `<match>` as a child of `<each>` is NOT rendered per-item — TWO defects: (a) the each per-item factory emits `// each: unhandled template child kind="match-block"` (the match is DROPPED), (b) a phantom MODULE-scope dispatcher is emitted instead, wired `_scrml_effect(() => __scrml_match_match_NN_dispatch(article.status))` where `article` is undefined at module scope | `emit-each.ts` doesn't handle the `match-block` template-child kind. R28-1 closed only the `@.`→loop-var gate-fire; this is the deeper architectural gap. "node-check-clean ≠ correct" silent-wrong/missing-output. Fix: emit the per-item match render INSIDE the each factory (item-scoped mount + dispatch) + suppress the module-scope dispatcher for match-in-each. emit-each.ts + emit-match.ts (+ emit-variant-guard.ts factored helper). |
