...
- **Every dev dispatch that writes scrml — gauntlet OR scrml-writer OR pipeline-doing-self-host — MUST include `docs/articles/llm-kickstarter-v0-2026-04-25.md` in the briefing.** Same reason as the anti-patterns brief but broader: the kickstarter gives the agent the canonical scrml shape, the stdlib catalog (kills npm reach), the inline anti-pattern table (every "if you'd reach for X in framework Y, use Z in scrml" mapping), and the recipes for auth/real-time/reactive. Derived from 5 clueless-agent experiments S41 (`docs/experiments/SYNTHESIS-2026-04-25-clueless-agent-runs.md`) — internal dispatched agents have the same training-data bias as external would-be-adopter agents. The brief must say: "Read `docs/articles/llm-kickstarter-v0-2026-04-25.md` in full before generating any scrml code." This complements the anti-patterns brief; both go in every relevant dispatch.
