...
2. **`g-s52-retraction-doc-staleness`** `[half-landed / half-escalated]` bug LOW · tier low — 2 derived docs still teach the deleted §52 auto-persist/optimistic/rollback model after the retraction (SPEC date **2026-06-14**, not "S194"). Reword to read-authority-only; initial-load/SSR claims REMAIN compliant — do NOT touch. Entry: components-are-states-devto + spec-consolidation heads-up. **R4-verified vs §52.6.2/§52.6.3.** **LANDED:** heads-up doc — 2 additive supersession notes (record-preserving). **ESCALATED:** components-are-states-devto is a public voice article whose central value-prop is now false — substantive reword in user voice (R1 + voice-author domain) + 2 mirrors; awaiting user direction. See progress.md.
3. **`r28-c2`** `[open]` bug MED · tier low — R28-C2 canon residuals: `< db>`/`< schema>` leading-space display-vs-copy-paste + `print()` residual. Doc-currency only (§11.3/§11.13 fixed S151). Per-site backtick-wrap + despace-in-fences (NOT a sed sweep). Entry: docs/articles/llm-kickstarter-v2-2026-05-04.md.
