...
1. `domain.map.md` — **"Native-Parser Swap Orientation"** block: the ~6-family flip-failure table (F1-F9, by locus + native file owner), which family is the NEXT dispatch, and the now-CLOSED each/match-promotion precondition. Read this FIRST for any native-parser flip-failure work.
...
### Native-Parser Swap — current state (S161 ratified / S162 re-measured)
- **The active strategic line**: drive `--parser=scrml-native` to DEFAULT, then DELETE legacy block-splitter (BS) + Acorn at M6 (direction-a, ratified S161). The native parser is `compiler/native-parser/` (paired `.js` bootstrap + `.scrml` self-host mirror; `--parser=scrml-native` flag).
- **CLOSED — each/match structural promotion (was the HARD M5-swap precondition, S153)**: as of S162 (39b1424a + d99403b1) the native parser NOW promotes BOTH `<each>` AND `<match>` to structural FileAST nodes: `<match>` → `match-block` (was already promoted via `isMatchBlock`/`synthMatchBlockNode`; the #2f survey proved byte-identical under flip), `<each>` → `each-block` (NEW: `isEachBlock`/`synthEachBlockNode` in parse-file.js, mirroring `synthMatchBlockNode`; carries the live `each-block` shape `{iterShape, inExprRaw, ofExprRaw, asName, bodyRaw, ...}`). `each` added to `STRUCTURAL_ELEMENTS` in tag-frame.js. The `@.` contextual iteration sigil is now lexed (unit C). emit-each.ts honors the `exprNode` contract for native per-item interp (unit B). **The S153 "does NOT promote" precondition is RETIRED — both each and match are native-structural.**
- **S162 flip re-measure**: ~790 flip-failures across 199 files (down from 1,150), triaged into **~6 parser FAMILIES, not 199 file-fixes**. Families by locus (full table in domain.map.md "Native-Parser Swap Orientation"):
  - **F1 engine arm-body parse (~168, L) — THE NEXT DISPATCH**: native fires spurious `E-UNQUOTED-DISPLAY-TEXT` on `<engine>` state-child arm bodies + DROPS the whole engine (+ each-in-arm). Locus: `parse-state-body.js` + the markup-classification path (`parse-markup.js` MK3.3 display-text detection + `tag-frame.js`/`parse-state-body.js` `tagKindFor` classifiers).
  - F2 SQL `?{}` in server-fn (~58) — `parse-sql-body.js` drops SQL body in top-level server fns.
  - F3 match/if-as-expr (~44) — same-line match arms DONE S162 (`parse-expr.js isAtArmBoundary`); if-as-expr residual.
...
- Native parser files (largest): `parse-stmt.js` (3990L), `parse-expr.js` (3956L), `parse-markup.js` (2916L), `tag-frame.js` (2402L), `parse-file.js` (1600L), `translate-stmt.js` (1686L), `parse-state-body.js` (235L — F1 locus). See structure.map.md "Native-Parser File Table (S162)" for the full per-file role map.
