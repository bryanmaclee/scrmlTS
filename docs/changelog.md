...
Current baseline (2026-05-05 S61): **8,878 pass / 44 skip / 0 fail / 8,922 across 439 files**. S61 landed Steps 11.5 + 12 (Phase A1a now **16/19 done**), the SPEC head broken-path cleanup, and Curation Batches A + C (19 dirs dereffed). Step 12 surfaced 2 NEW parser-gap follow-ups (Steps 11.0d + 11.0e) — BRIEFs queued. Remaining A1a: 11.0d, 11.0e, 13 (~4.5-9.5h).
...
- **Curation pass started** — `docs/changes/` 103-dir wholesale review per pa.md "current truth only" scope principle. Disposition matrix at `docs/curation/2026-05-05-changes-dir-disposition.md` (10 batches; PA proposes per-dir disposition; user ratifies per-batch).
  - **Batch A (P-series, 12 dirs)** RATIFIED + EXECUTED (`f4c0081` scrmlTS / `df2f3d2` scrml-support). 12 S40-era foundational dispatches dereffed to `scrml-support/archive/changes/`: p1, p1.e, p2, p2-wrapper, p3.a, p3.a-follow, p3.b, p3-error-rename, p3-follow, p3-rename, p3-spec-paperwork, p4-scrml-migrate. Cross-ref in `examples/23-trucking-dispatch/FRICTION.md` updated.
  - **Batch C (dispatch-app M-series, 7 dirs)** RATIFIED + EXECUTED (`729e57c` scrmlTS / `9943174` scrml-support). 7 S38-39 multi-stage dispatch dirs dereffed: dispatch-app, dispatch-app-m1..m6. 2 cross-refs in `examples/23-trucking-dispatch/README.md` updated (header status box + Links section).
  - 8 batches remaining: B (expr-ast-phase-4d, 4), D (F-series, 11), E (GITI, 2), F (BUG-letters, 2), G (bun-sql, 2), H (LSP L1-L4, 5), I (fix-*, 20), J (misc, 12). `docs/changes/` count: 103 → 84.

- **Maps refresh attempted** — S61 maps-refresh agent ran cold scan but Write tool returned permission-denied (system-level directive). Agent returned thorough findings as text including 8 categories of non-compliance items. Item #1 (SPEC head cleanup) actioned. Item #2 (`docs/changes/` curation) in progress (Batches A + C done). The maps files themselves remain stale (last touched 2026-04-24); root-cause investigation deferred to next session.
