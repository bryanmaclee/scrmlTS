...
status: candidate     # the markup-lease debate was banked S206 OUTSIDE this queue (Bucket-B note only) + the block-lease W3.5 coupling banked S221 → NEVER formally queued; this entry CONSOLIDATES them + makes the whole "fully subsuming block-lease" thread drainable (S225 capture, user "make sure all stagnant DDs/debates can drain").
...
status: candidate     # banked S207; GATED on flogence maturity — premature to judge until flograph/dock coverage is high enough.
