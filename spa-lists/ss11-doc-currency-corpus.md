...
1. **`bug-19-cite`** `[open]` bug LOW · tier low — §11-folded-citation sweep: 5 dev.to articles cite stale SPEC §11 for `<db>`/protect=/state-authority (folded → §6.12+§52); E-code citations correct. Entry: docs/articles/*-devto-*.md (index.html mirrors regenerate).
2. **`g-s52-retraction-doc-staleness`** `[open]` bug LOW · tier low — 2 derived docs still teach the deleted §52 auto-persist/optimistic/rollback model after the S194 retraction. Reword to read-authority-only; initial-load/SSR claims REMAIN compliant — do NOT touch. Entry: components-are-states-devto + spec-consolidation heads-up.
