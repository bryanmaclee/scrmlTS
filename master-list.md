...
**Last updated:** 2026-05-11 (S82 — **Doc-system audit + maps-discipline protocol · IMPLEMENTATION-ROADMAP.md + IMPACT-ASSESSMENT.md banner-marked ⛔ HISTORICAL with redirect to current SoT (master-list §0 / changelog / hand-off / SPEC) · master-list named explicit session-start SoT in pa.md checklist · 5 shipped change/audit dirs dereffed to scrml-support/archive · master-list §0.5 collapsed (538 → 512 lines) · primary.map.md gained Task-Shape Routing + Use-feedback-loop sections · pa.md §"Maps-discipline protocol (S82)" added (dispatch-brief template + currency check + map-selection ownership + feedback-loop disposition + losing-battle threshold) · project-mapper agent template extended for future regeneration · 7 commits across 2 repos · 0 source changes · 0 regressions · pushed · 1 day**)

**Tests (current — S82 close, pre-commit subset):** **10,458 pass / 66 skip / 1 todo / 0 FAIL** (507 files; `bun test compiler/tests/{unit,integration,conformance} --bail` ~12s). **Full suite (S82 close, `bun run test` incl. browser/lsp/commands/self-host):** **11,259 tests total / 0 FAIL** (535 files). +25 pre-commit pass over S81 close is incidental conformance fluctuation since 0 source code changed this session. Pre-commit hook firing every commit (installed S79); cross-machine sync clean.
