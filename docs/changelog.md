...
Current baseline (2026-05-07 S66 close): **9,090 pass / 44 skip / 1 todo / 0 fail** (full suite); **8,366 pre-commit subset**. +71 net from S65; 0 regressions. **A1b foundational wave 1 COMPLETE** (B1-B5 all landed; B4 + B6 added in S66). **Promotion ergonomics Tier B SHIPPED on full predicate matrix** (`is` AND `==`, mixed) after S66 narrowing-error reversal + parser fix made bare-dot variants `.Variant` parseable as primary expressions everywhere. **Tier C scoped** (`--engine` + W-MATCH-TRANSITIONS-ACCRUING). **Self-host plan changed**: deferred to post-v1.0.0; entire self-host scrml compiler will be human-authored, processed through scrmlTS. **pa.md "Design discipline" section added** with 4 PERMANENT rules until v0.2.0 ships: (1) no marketing PA-volunteered work; (2) scrml not a toy; (3) right answer beats easy answer 99.999%; (4) spec is normative — derived planning docs are NOT. **Maps cold-start refresh** (first since S40). **A1c + B7 + B8 Rule-4 audits** landed pre-emptively. Next: B7 dispatch + B8 dispatch + A1b wave 3 (B9-B13 validators); workflow concern surfaced for S67 deliberation.
