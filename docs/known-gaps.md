...
### Bug 61 — `@compound.isValid` (and §55 compound-level rollup synth reads) emit member-access on the compound proxy, not `_scrml_derived_get(dotted)` — `NEW S140; HIGH; OPEN` (PA-verified; surfaced by Bug 58)
...
### Bug 58 — `formFor` validity surface never emitted (synth compound state-decl spliced into markup-children) — `RESOLVED S140 (commit 29c33a6c)` (was HIGH; PA-verified, happy-dom gate) — NB read-path sibling Bug 61 still OPEN
