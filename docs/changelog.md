...
Current baseline (2026-05-22 **S121 CLOSE**). Full `bun run test` **19,774 pass / 0 fail / 171 skip / 1 todo across 746 files**; pre-commit gate (unit+integration+conformance) **13,773 pass / 0 fail / 88 skip / 1 todo across 704 files**. Pre-push gate (full validation + TodoMVC) green on every landing. **The C2 gap ledger closed 16 → 2 (998/1000 strict-pass = 99.8%)** — every actual parser-side bug is now closed; remaining 2 residuals are corpus-stale only (bs.scrml null + quiz-app `</>` operator), deferred to M6-gated corpus sweep. **M6 mechanical preconditions cleared**: `is not not` (36 sites), E-FN-003 (178 sites across 5 .scrml mirrors), E-EQ-004 (46 fires from display-text-literal.scrml), W-DEAD-FUNCTION (20 fires via compiler-source RI walker root-cause fix). **3 NEW canary classes shipped** crediting native correctness when live is the broken oracle: LIVE-PHANTOM (W6-B), LIVE-HOIST-MISCLASSIFY (W9-H), LIVE-DEGENERATE threshold relax (W8-G); all sunset at M6. **Bug 8 closed end-to-end** — 13 missing `scrml:*` runtime shims + W-STDLIB-SHIM-MISSING + scrml:compiler family deferral hardening (13 thunks + W-STDLIB-COMPILER-DEFERRED + SPEC §41.17 NEW). **Bug 9 filed** — dashboard runtime exercise surfaced async-not-awaited codegen class; defer to post-M6.
