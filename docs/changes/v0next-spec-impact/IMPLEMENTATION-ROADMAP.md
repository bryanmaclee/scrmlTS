...
- Event-timeout watchdog (no-event-for-N-ms) — per-machine last-event-timestamp tracker; only loads when an engine declares an event-timeout. **SHIPPED at S77 2026-05-10** — `<onIdle after=DURATION to=.Variant/>` element at engine-root scope. SPEC §51.0.R; 3 new §34 codes (E-IDLE-DUPLICATE / E-IDLE-INVALID-VARIANT / E-IDLE-MISPLACED); per-engine watchdog config emitted as `__scrml_engine_<varName>_idle`; 5th-arg `idleEntry` threaded through write-guard call sites; reset-on-commit; tree-shake when no `<onIdle>` per engine. Computed-delay form works (rides parseAfterDuration helper).
- Named multi-timer-per-state (gen_statem-style) — `Map<Name, Handle>` only loads when an engine declares named multi-timers. **DEFERRED S77** — depends on engine state-child body rendering (currently emits a placeholder marker per `emit-engine.ts:46,669,1175,1478`). Without body-render, `cancelTimer("name")` builtin has no calling surface inside engine state-child bodies. Defer until body-render lands; the named-timer surface can be revisited then.
