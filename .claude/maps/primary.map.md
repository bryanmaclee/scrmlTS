...
| Map                  | Status  | Contents                                                      |
|----------------------|---------|---------------------------------------------------------------|
| structure.map.md     | present | directory layout, entry points, S148-S159 source changes (engine-graph, source-map, _scrml_modules, dev watcher, Shape 4, S153 each-in-dynamic-context, S154 message-arm parser, S155 #14 typer+codegen, S156 Bug 62 engine-ctx + (d)-A enum-subset 4 batches, S157 Bug 60/63/65/67/68/70/71 + match-exhaustiveness, S158 Bug 64/R28-1c per-item-reactivity + Bug 72 nested-each-in-lift, S159 Bug 73 per-item-handler-live-keying + S154 ruling (a) HTML colon-shorthand content-model) |
| dependencies.map.md  | present | 9 packages (3 runtime root + 2 compiler + 4 devDeps), internal graph (HEADER STALE — content not touched by S154-S159, last refreshed 4e1f9492) |
...
| error.map.md         | present | 380+ error codes; +5 new S154-S156 codes (E-ENGINE-ACCEPTS-NOT-ENUM, E-ENGINE-MSG-WITHOUT-ACCEPTS, E-ENGINE-MSG-ARM-NOT-EXHAUSTIVE, E-ENGINE-MSG-UNKNOWN, E-MATCH-SUBSET-DEAD-ARM); S157 E-SYNTAX-064 promoted to explicit; E-CODEGEN-INVALID-JS Bug 70 suppression; **S159 +1** E-COLON-SHORTHAND-ON-VOID (void HTML element rejects `:`-shorthand, §4.14 / §34); S159 E-SYNTAX-064 extended to `:`-shorthand body positions |
| test.map.md          | present | bun:test, ~880 .test.js files; +11 S154-S156 unit+browser+conformance tests; +14 S157-S158 unit+browser tests; **+3 S159** (per-item-handler-live-keying-bug73 unit+browser, html-colon-shorthand-content-model-s159 unit) |
| domain.map.md        | present | 12-stage pipeline + sidecar, 30+ domain concepts incl. S153 each-in-dynamic-context, S155 `accepts=`/message-dispatch/#14 plane, S156 Bug 62 engine-ctx threading, S157 Bug 65 CLOSED (lift engine-ctx) + Bug 60 CLOSED (render-by-tag compound) + match-exhaustiveness arc, S158 Bug 64/R28-1c CLOSED (per-item-reactivity + live-keyed) + Bug 72 CLOSED (nested-each-in-lift), **S159 Bug 73 CLOSED** (per-item-handler-live-keying, both tiers) + **S159 S154 ruling (a)** HTML `:`-shorthand content-model rule (non-void renders / void rejects); codegen each/match/engine emit map with updated line counts |
...
| business invariants (null-not-in-scrml, auth-content-not-gated, arm-separator, Shape 4, each-chunk-survival, dep-first-read, engine-ctx-threading, subset-range-forbidden, per-item-live-keyed, per-item-handler-live-keyed, _scrml_resolve_item-null-not-undefined, E-CODEGEN-suppressed-on-prior-fatal, void-rejects-colon-shorthand) | domain.map.md |
| `<each>` / `<match>` / engine codegen emit modules | domain.map.md "Codegen each/match/engine Emit Map" + structure.map.md S154-S159 |
...
3. `structure.map.md` — the "Key S154-S159 Source Changes" section gives exact file + function + line for recently-touched code; the codegen directory ownership line names the emit-* module
...
2. `structure.map.md` — S154-S159 source-changes section: exact functions + line numbers in emit-each.ts / emit-lift.js / emit-control-flow.ts / emit-engine.ts / emit-html.ts / runtime-template.js / dependency-graph.ts / ast-builder.js / block-splitter.js / type-system.ts
...
1. `domain.map.md` — pipeline stage (BS/TAB) + engine-arm-parsing row; the native-parser M5-swap precondition (does NOT promote each/match → structural nodes); S159 `:`-shorthand HTML content-model concept
2. `structure.map.md` — engine-statechild-parser.ts S154 `parseMessageArms()` + S153 `isColonShorthandOpener` change + native-parser directory note; S158 ast-builder `_parseLiftAttrValue` bare-`@` branch; S159 ast-builder `buildBlock` synthesis + block-splitter.js `shorthand && !selfClosing` reorder
3. `error.map.md` — E-ENGINE-STATE-CHILD-MISSING / E-ENGINE-ACCEPTS-NOT-ENUM / E-ENGINE-MSG-* / E-CTX-* / E-EXPR-* / E-STMT-* / E-SYNTAX-064 / E-COLON-SHORTHAND-ON-VOID families
4. `test.map.md` — parser-conformance within-node allowlist (live-pipeline vs native-parser parity); S159 html-colon-shorthand-content-model-s159.test.js
...
- Spec: `compiler/SPEC.md` (31,494 lines, 58 sections + appendices); normative per pa.md Rule 4; §4.14 amended S159 to define HTML element `:`-shorthand content-model (non-void renders / void rejects + E-COLON-SHORTHAND-ON-VOID)
...
- type-system.ts is 17436 lines (largest single source file); type-checking, linear types, validity-surface synthesis, enum-subset resolution, match-as-expr exhaustiveness, markup-attr `.advance` two-plane check, E-COLON-SHORTHAND-ON-VOID guard (S159)
...
## Tags
#scrmlts #map #primary #compiler #bun #v0.7.0 #each-in-dynamic-context #codegen #enum-subset #message-dispatch #per-item-reactivity #live-keyed #r28-1c #bug60 #bug62 #bug63 #bug64 #bug65 #bug70 #bug71 #bug72 #bug73 #colon-shorthand-html #s148 #s149 #s150 #s151 #s152 #s153 #s154 #s155 #s156 #s157 #s158 #s159
