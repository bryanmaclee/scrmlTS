 ...
 *
 * Run from repo root: node docs/changes/lin-batch-a/apply-patch.js
 * Idempotent: patch markers prevent double-application.
 */

// Using CommonJS to avoid ESM url/dirname complexity in Node 22
...
function applyChanges(filePath, changes, fileLabel) {
  ...
  for (const change of changes) {
    if (content.includes(change.marker || change.old.slice(0, 60))) {
      if (change.marker && content.includes(change.marker)) {
        console.log("  [SKIP] " + change.name + " — already applied");
        continue;
      }
    }
    if (!content.includes(change.old)) {
      console.error("  [FAIL] " + change.name + " — old text not found. Aborting.");
      process.exit(1);
    }
  ...
  if (written > 0) writeFileSync(filePath, content, "utf8");
  return written;
}

// ══════════════════════════════════════════════════════════════════════════════
// Build change objects — using single-quoted strings to avoid template literal
// escaping collisions with the TypeScript source that contains backtick strings.
// ══════════════════════════════════════════════════════════════════════════════

const tsPath = join(repoRoot, "compiler/src/type-system.ts");

// Exact source strings from type-system.ts, reproduced verbatim:
const c1_old =
...
'    if (!err) {\n' +
'      // First consumption — mark as lift-sourced.\n' +
...
// Change 3: emitLinError E-LIN-002 — backtick strings require careful escaping.
// The TypeScript source uses template literals with backtick + ${}. We reproduce
// those literally here using single-quoted JS strings.
const c3_old =
...
'        // AST shape: lift-expr has expr: { kind: "expr", expr: "<expression string>" }.\n' +
'        // Detect bare-identifier lift targets and consume them from the lin tracker.\n' +
...
'          // Check local and parent lin trackers for a matching lin variable.\n' +
...
'      // in loopLocalLin, not the outer tracker. Allows declare+consume within\n' +
'      // a single loop iteration without triggering E-LIN-002.\n' +
...
'          continue;\n' +
'        }\n' +
'        // Not in loopLocalLin — falls through to walkNode for outer-scope rejection.\n' +
'      }\n' +
'\n' +
'      walkNode(node, lt, tt, /* inLoop= */ true);\n' +
'    }\n' +
'\n' +
'    // Lin-A3: Unconsumed loop-local lin vars → E-LIN-001 (not consumed within iteration).\n' +
...
// ══════════════════════════════════════════════════════════════════════════════
// Test file — append Lin Batch A tests
// ══════════════════════════════════════════════════════════════════════════════
...
'    const e = errors.filter(e => e.code === "E-LIN-002");\n' +
'    expect(e.length).toBeGreaterThan(0);\n' +
'    // Message must mention "lift" and the lift site line number (7)\n' +
...
'// ---------------------------------------------------------------------------\n' +
'// Finding: the "~ double-obligation trap when lifting lin values" (hand-off-134.md)\n' +
'// is addressed by the combination of:\n' +
'//   1. Existing E-TILDE-002 for the ~ accumulator side (already tested in §41)\n' +
'//   2. New Lin-A1 fix for the lift-of-lin-var side (tested above)\n' +
'// No additional source changes are needed for Lin-A2. Status: ADDRESSED.\n' +
'\n' +
...
'    // Developer lifts a lin var (expecting a value back), then re-uses it.\n' +
'    // This is the double-obligation trap described in hand-off-134.md.\n' +
'    const errors = [];\n' +
'    checkLinear([\n' +
...
// ══════════════════════════════════════════════════════════════════════════════
// SPEC.md — §34.4.4 carve-out normative text (Lin-A3)
// ══════════════════════════════════════════════════════════════════════════════
...
'lin token = mintToken()\n' +
'for (const item of items) {\n' +
'    submitOne(token)  // Error E-LIN-002: outer lin var consumed inside loop\n' +
'}\n' +
'\n' +
...
console.log("  cd compiler && bun test 2>&1 | tail -20");
console.log("  git add compiler/src/type-system.ts compiler/tests/unit/type-system.test.js \\");
console.log("          compiler/SPEC.md docs/changes/lin-batch-a/");
console.log("  git commit -m 'feat(lin-batch-a): lin type improvements (A1/A2/A3)'");
