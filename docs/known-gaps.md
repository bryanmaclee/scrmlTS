...
### G-NOT-NEGATION-UNENFORCED — `E-TYPE-045` (`not` used as boolean negation) is SPEC-`SHALL`-error but the compiler accepts it silently — `PARTIAL S188 (5/6 positions landed); MED; residual: bare-not in ATTR values`
<!-- @gap id=g-not-negation-unenforced sev=MED status=open -->
...
**S188 PARTIAL LANDING (`g-not-negation-enforce-2026-06-12`, agent a2cd30a3, FINAL_SHA 1d24f62):** enforcement wired at the expression-parser lowering choke-point (`preprocessForAcorn` stamps `_notPrefixNegation` → `harvestNotPrefixNegation` TS-J emits E-TYPE-045); coupled corpus migration of 62 forbidden sites → `!` across 35 files (flagship + examples + samples + auth template); SPEC §34/§42.10 broadened bare+paren/all-positions; full suite 23982/0. **KEPT OPEN — PA independent R26 (S138 dual-verify) caught a RESIDUAL HOLE the agent's R26 missed: bare (non-parenthesized) prefix-`not` in ATTRIBUTE values does NOT fire E-TYPE-045.** Isolated: `<p if=not @y>` (h1) ✗ · `<p if=@x && not @y>` (h2) ✗ · `<p show=not @y>` (h6) ✗ all silent-compile; but `<p if=(not @y)>` PAREN-in-attr (h5) ✓ fires · `${ @a && not @b }` bare-in-LOGIC (h3) ✓ · interpolation (h4) ✓ · ternary (f1/f2) ✓ · if-STATEMENT golden ✓. Root: bare-`not` in an attr value routes through a path (likely codegen `rewriteNotKeyword`/`rewrite.ts` — Agent-3 'Locus 3') that bypasses the `parseExprToNode` stamping choke-point; the agent's Locus-2-only impl + its 'all-positions' test both missed the attr-bare position. **Follow-up dispatched** to close the attr-value bare-`not` hole + add the missing test coverage; gap flips RESOLVED then.
...
### G-DIVISION-IN-TERNARY-ARM — a `/` division inside a ternary arm (`cond ? a / b : c`) → E-CODEGEN-INVALID-JS in all positions — `NEW S188 dog-food; MED; /-as-regex ambiguity in ternary-arm context`
<!-- @gap id=g-division-in-ternary-arm sev=MED status=open -->

**Surfaced S188 dog-food (channels §38 → derived/expr sweep).** A `/` division operator inside EITHER arm of a ternary — `@e > 0 ? @h / @e : @h` — emits invalid JS (`E-CODEGEN-INVALID-JS`, "compiler defect"). The `/`-as-regex disambiguation in the shared regex/string fence (`code-segments.ts` / `rewriteCodeSegments`) mis-classifies the arm-position `/` as a regex-literal start (after `?`/`:` the parser is in expect-expression mode), then mangles the rest of the ternary. **Isolated (verify-before-claim):** `@h / @e` standalone (c3) → CLEAN; ternary with literal/string arms `@e>0 ? 1 : 2` (c2/c4) → CLEAN; ternary with `*` in an arm (`@a>0 ? @a*2 : @a`, tmul) → CLEAN; ternary with `/` in CONSEQUENT (cx) OR ALTERNATIVE (cy) arm → FAIL. **Broad across positions:** fails in a local `const x = …` in a `${}` block, a DERIVED cell `const <ratio> = …` (tdiv_derived), AND a markup interpolation `${ … }` (tdiv_interp). **Severity MED** — the divide-with-guard idiom `cond ? a/b : fallback` (guard against divide-by-zero, a textbook pattern) breaks everywhere it is idiomatically written; loud-not-silent; clean workaround = parenthesize the division (`(@h / @e)` → confirmed clean) or restructure. NOT the `/not found/i` regex case (that is fenced correctly — T1/T4 of the same sweep). **Fix direction:** the `/`-disambiguation heuristic must recognize that a `/` whose preceding token is an operand (`@e`, `)`, `]`, identifier, literal) is DIVISION even in ternary-arm position — the same operand-precedes-`/` rule that already works outside ternaries (c3). Repros: `/tmp/dfn/cx.scrml`,`cy.scrml`,`tdiv_derived.scrml`,`tdiv_interp.scrml` (fail) vs `tmul.scrml`,`c3.scrml`,`wc.scrml` (clean).
