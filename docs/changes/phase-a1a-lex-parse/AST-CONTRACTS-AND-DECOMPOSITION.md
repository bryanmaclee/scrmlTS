...
| **11.0d** | **Top-level structural Shape 1 recognition** (P-FUP-1 from Step 12) | BS top-level scan extension via `peekTopLevelStateDeclSignal` (ast-builder.js + block-splitter.js) + 3 sample restorations + §S11D test block | **3-6 h** | ✅ DONE S61 (`0f92077`, T2 tier; +9 pass / +1 todo; 9 §S11D positive cases + 1 .todo for Variant C compound at top-level deferred; 3 reverted Step 12 samples restored; component-def discrimination preserved; original + finisher agents both stalled with stream timeouts — PA salvaged) |
| **11.0e** | **`<x> = not\n<y>` newline-as-separator boundary fix** (P-FUP-2 from Step 12) | added `"not"` to `VALUE_KEYWORDS` Set at ast-builder.js L1970; preserves Step 11.0b universality | **1-3 h** | ✅ DONE S61 (`916de65`, T2 tier; +8 tests; 4 of 5 reverted Step 12 samples restored; 1 LOC code change + 10 LOC explanation comment; UNIVERSAL fix; surfaced P-FUP-3 → Step 11.0f) |
| **11.0f** | **`<x> = ?{SQL}\n<y>` BLOCK_REF newline-as-separator boundary fix** (P-FUP-3 from Step 11.0e) | extend `lastEndsValue` value-classifier in collectExpr ASI-NEWLINE branch to cover BLOCK_REF; restore combined-007-crud sample + new positive cases | **1-3 h** | ⏸ NEW S61; BRIEF at `docs/changes/phase-a1a-step-11-0f-blockref-newline-boundary/BRIEF.md` |
| 13 | Final commit + CHANGELOG draft | aggregate; final commit per brief §7.4 template; cleanup `scripts/step12-*.mjs` helpers | 0.5 h | ⏸ |

**Total focused work landed: ~50-65h across S59 + S60 + S61.** ALL 20 sub-steps (Steps 1-13 + 11.0a/b/c/d/e/f + 11.5) DONE at S61 close. **Phase A1a (lex+parse) COMPLETE.**
