...
Surfaced by the Bug 65 dispatch (S157) while building the engine-ctx threading test fixtures. A Tier-0 `${ for (it of @items) { lift <li onclick=ping(@.id)>…</li> } }` — where the lifted CALL-REF event handler passes the iteration-local `@.` sigil as an argument (`ping(@.id)`) — emits the raw `@.` (the `@.` iter-scope lowering does NOT run in the for-lift call-ref handler-arg path), producing invalid JS that trips `E-CODEGEN-INVALID-JS`. Reproduces on the ORIGINAL pre-Bug-65 code (NOT a Bug 65 regression; NOT engine-related — it's a Tier-0 for-lift `@.`-arg-lowering gap in the call-ref handler path, parallel to the engine-ctx gap Bug 65 closed but for the iter-sigil rather than the engine ref). The Bug 65 §5 tree-shake test uses `ping(it)` (the `as`-name alias) to sidestep it. LOUD-fail (compile error, not silent) — but `onclick=handler(@.id)` (passing the current item's id to a handler) is a common pattern, so an adopter hits it. Fix = run the `@.` iter-scope prelowering (the `rewriteIterScopeOnly` machinery the each-path already uses) on for-lift call-ref handler args. MED. Cross-ref Bug 65 (the engine-ref sibling, RESOLVED S157); the each-path `rewriteIterScopeOnly` (`emit-each.ts`) is the template. Also noted-not-fixed: `emit-lift.js:~1088` `emitCreateElementFromExprString` (bare-tag short-form, no closer) does not thread `engineCtx` (left engine-free; not reachable by the for-lift markup engine-handler path today — LOW edge).
