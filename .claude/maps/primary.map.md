...
| test.map.md          | present | bun:test, 912 .test.js files (23,054 pass / 0 fail at S165, within-node 1005/0); +5 S162; **+4 S164 native-parser unit** (native-attrvalue-exprnode-population, native-lift-markup-closetag-span, native-sql-chained-form-f2a, native-tablefor-struct-field-drop) + within-node/conformance updates S164-S165 |
| domain.map.md        | present | 12-stage pipeline + sidecar, 30+ domain concepts incl. S153 each-in-dynamic-context, S155 `accepts=`/message-dispatch/#14 plane, S156 Bug 62 engine-ctx threading, S157 Bug 65/60 + match-exhaustiveness, S158 Bug 64/R28-1c + Bug 72, S159 Bug 73 + S154 ruling (a); **S162-S165 native-parser-swap orientation block (~5-family flip-failure table; each/match-promotion CLOSED S162; F1 engine-substrate CLOSED S163; §51.0.S message-arm/exprNode/F2-match/promote-each/R1 typed-@cell/server-fn-star LANDED S164-S165; flip 674→451)**; codegen each/match/engine emit map |
...
4. `error.map.md` — E-UNQUOTED-DISPLAY-TEXT (§4.18.7 SPEC-CORRECT native-strict, NOT spurious — corpus migration deferred), E-ENGINE-ACCEPTS-NOT-ENUM (no longer mis-fires post-B2), E-ENGINE-MSG-* / E-CTX-* / E-EXPR-* / E-STMT-* / E-STRUCTURAL-ELEMENT-MISPLACED / E-SYNTAX-064 families; the each-in-match historical-fix note (S162-native-stale, already annotated)
5. `test.map.md` — parser-conformance within-node allowlist (1005/0); the flip-harness (default exit-0 vs `--parser=scrml-native` fail/miscompile) is the family-level gate — **re-run it to re-rank the remaining 451 before picking a family**; S164 native-parser unit tests (exprNode-population, lift-closetag-span, sql-chained-f2a, tablefor-field-drop)
...
### Native-Parser Swap — current state (S161 ratified / S163 engine-substrate fix / S164-S165 parity-closer grind)
...
- **Flip re-measure: 451 flip-failures at S165 close** (674 S164-open → 509 S164-close → 451 S165; from ~790 S162, 1,150 S161). Full suite at S165: 23,054 pass / 0 fail / 912 files; within-node 1005/0. Default output UNCHANGED (no version bump S162-S165 — every closer is shadow-only). LANDED S164 (674→509): §51.0.S message-arm + exprNode-walker + lift-closetag + F2a + table-for. LANDED S165 (509→451, −58): F2-match + promote-each + R1 typed-`@cell` + server-fn-star. ~8 environmental; remaining ~5 families (full table in domain.map.md "Native-Parser Swap Orientation"):
...
- Native parser files (largest, S165 line counts): `parse-stmt.js` (4201L — S165 `parseTypedAtStateDecl`/§17.4a-b/`typeBodyText`), `parse-expr.js` (3983L — S165 `StringLit` match-arm), `parse-markup.js` (3097L — S165 `BARE_DECL_RE` generator sync + S164 F1-narrow), `tag-frame.js` (2402L), `translate-stmt.js` (2109L — S165 for-stmt synth + `Yield`/`reconstructChainedSql`), `parse-file.js` (1671L — `collectMachineDeclsFromNodes` engine-substrate share, S163), `translate-expr.js` (1167L — S165 Literal arm + B1 reset-expr S163), `collect-hoisted.js` (872L — `synthEngineDecl` reads `accepts=` S164, no longer synthesizes engines S163), `ast-stmt.js` (605L), `ast-expr.js` (493L — S165 `MatchArmPatternKind.Literal`). **B2 (CLOSED S164) loci:** `collect-hoisted.js synthEngineDecl` (`accepts=` read) + `native-walker/engine-statechild-walker.ts` (`messageArms` from `parseMessageArms`). **Native attr-value `exprNode`/`argExprNodes`:** stamped by `compiler/src/native-walker/attrvalue-exprnode-walker.ts` (run from api.js:945), NOT in the native-parser/* tree. See structure.map.md "Native-Parser File Table (S165)" for the full per-file role map.
