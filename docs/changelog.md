...
Current baseline (2026-05-19 **S107 CLOSE** — 9-commit grind: dogfood-bug cascade (5/3/6) + match-block-form spec-vs-impl gap discovery + impl-arc Phases 1+2 SHIPPED + adopter-direct Known gaps surface + README designer-note + rule= clarification): pre-commit subset **13,087 pass / 88 skip / 1 todo / 0 fail / 681 files / 44,430 expect**; full `bun test compiler/tests/` **15,930 pass / 169 skip / 1 todo / 0 fail / 714 files / 46,845 expect**. Delta vs S106 close (15,867 / 710 / 46,721): **+63 pass / +4 files / +124 expect / 0 fail / 0 regressions** (59 new tests + 4 fixture-shape edits to existing). **Headlines:** (1) **Bug 5 Phase 1+2 SHIPPED** — closed dogfood Bug 5 HIGH-severity `${IDENT}` non-reactive interpolation no-op (emit-event-wiring.ts missing-else-branch + one-shot DOMContentLoaded textContent write) + Anomaly B (orphan `IDENT;`/`_scrml_reactive_get("count");` no-op JS at file-scope) + Anomaly C (phantom `<span data-scrml-logic>` from decl-only logic bodies); 26 unit tests; 17 regressions surfaced + fixed via kind-guard + 4 brittle pre-existing tests counter-resilient'd. Phase 3 polish (constant-folding + SPEC §7.4.2 + tilde threading + multi-binding dedup) queued ~5-8h. (2) **README "A note from the designer"** — user-authored personal note with `~96%` AI-disclosure + 3-years-of-compiler-self-study backstory + closing "are the ideas any good?" question. (3) **Bug 3 SHIPPED** — `[BS]` / `[TAB]` diagnostics now carry `path:line:col` prefix matching W-LINT-* convention (api.js collectErrors enriched + bsSpan→span normalization + dev.js / build.js formatter mirror); 6 tests. (4) **Bug 6 SHIPPED** — 2 hallucinated error-code references in `docs/website/pages/` retired (E-ENGINE-INCOMPLETE-COVERAGE → E-ENGINE-STATE-CHILD-MISSING + E-PURE-VIOLATION → E-PURE-001); side-session-predicted retired-rename drift had ZERO hits — methodology validated Rule 4 (canonical catalog had answers; predicted-drift list was prior-mental-model). (5) **Match-block-form impl arc Phases 1+2 SHIPPED** — README rule= clarification investigation traced silent acceptance of `<match>` block-form to opaque html-fragment fallthrough; entire SPEC §18.0.1+§18.0.2+§18.0.3 spec'd-but-unparsed; full SCOPING (5-phase plan ~12-19h + 10 OQs / 4 ratified) + Phase 1 (one-line BS exempt-list fix + 145-line ast-builder dispatch produces structured `match-block` AST node + 9 tests) + Phase 2 (STRUCTURAL_RAW_BODY_ELEMENTS BS gate + match-statechild-parser.ts NEW 440 lines recognizing 3 body forms + wildcard + payload bindings + new SYM PASS 20 firing ALL 5 diagnostics + SPEC §34 +1 row + §18.0.1 normative bullet + 18 tests). README rule= clarification now TRUE end-to-end. Phase 3 codegen + Phase 4 bare-variant inference + Phase 5 samples queued. (6) **NEW `docs/known-gaps.md` adopter-direct surface** — user-direction "honest current state, link to error log, major ones called out on front page"; curated spec-vs-impl drift list with severity + status + workarounds + SCOPING links; README current-state blockquote adds "Known gaps" paragraph; closes mouth-to-reality framing ("v0.3.0 stable was overclaimed if 'stable' means every spec'd surface implemented").
