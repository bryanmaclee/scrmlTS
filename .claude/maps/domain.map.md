...
| value-native map (§59, SPEC-AHEAD S168) | (`4c8063b6`): NEW SPEC §59 — a `[KeyT:ValT]` map is reassignment-canonical / COW (a write returns a new map: `@m = @m.insert(k, v)`), mirroring arrays. Reads are bracket (`@m[k]` → `V | not`); writes are method (`E-MAP-BRACKET-WRITE` on `@m[k] = v`). Keys identified by structural `==` (key type must be §45-comparable). Maps are order-independent-comparable; `@ordered` postfix affix opts into iteration order. **Nominal — SPEC-TEXT ONLY; no type-system/parser/runtime/codegen yet (phase-c flips the banner).** The cycles-prereq above is its enabling groundwork. |
...
| compiler/src/runtime-template.js (**3776L** at S168) | `_scrml_each_renderers` registry + `_scrml_remount_each` helper (S153); `_scrml_engine_dispatch_message` (S155); `_scrml_reconcile_list` + `container._scrml_item_by_key` key→item map + `_scrml_trigger(container, "_scrml_items")` (Bug 64 S158); `_scrml_resolve_item(container, key)` returns live item via `_scrml_deep_reactive` or null (Bug 64 S158; also consumed by Bug 73 handler re-resolution prelude); **S168 cycles-prereq: `_scrml_structural_eq(a,b,seen)` WeakMap/WeakSet cycle guard (~L2491) + `_scrml_deep_set` COW clone-then-set is the bracket-write target**; `_scrml_effect_static`, `_scrml_modules` |
...
## Native-Parser Swap Orientation (S161 ratified / S165 re-measured / S166 two roots closed) — READ FIRST for any native-parser flip-failure work
...
within-node-classifier STRIP_KEYS). ~8 environmental; remaining ~5 families below.
