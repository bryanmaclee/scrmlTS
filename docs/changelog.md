...
Current baseline (2026-05-05 S60 close): **8,853 pass / 43 skip / 0 fail / 8,896 across 439 files**. S60 was a heavy-dispatch session — Phase A1a 12/17 done (Steps 6, 7, 9, 10, 11, 11.0a landed in S60). A1b + A1c scope-out docs drafted as durable artifacts. ADR ratified Option A FOLD (`reactive-derived-decl` → `state-decl{shape:"derived"}`) as Step 11.5. Step 11 surfaced 3 deferred parser gaps (11.0a/b/c); 11.0a landed; 11.0b + 11.0c BRIEFs drafted. Depth-of-survey discount #8 confirmed (Step 10 zero-source). All commits pushed.

### 2026-05-05 (S60 — A1a 5 steps + 11.0a landed + A1b/A1c scope-out + ADR ratification)

S60 opened on a clean baseline (8,784) post-S59 close with Phase A1a 7/13 done. Six dispatch cycles + extensive planning produced: 6 A1a step landings (Steps 6, 7, 9, 10, 11, 11.0a — net **+125 tests**), full scope-out documents for A1b (22 steps, 85-120h) and A1c (23 steps, 93-131h), ADR ratification for `reactive-derived-decl` divergence (Option A FOLD, sequenced AFTER Step 11 BEFORE Step 12, inserted as Step 11.5), Step 11 escalation discovery (3 deferred parser gaps surfaced; 11.0a landed; 11.0b/c BRIEFs drafted), and decomposition refresh (A1a now 17 steps including 11.0a/b/c + 11.5).
...
- **Step 11.0a — Variant C compound recognizer** (`6d51d00`). 3 commits; +127 LOC source + 14 LOC types. BRIEF touchpoint correction (L3528-3580 was wrong; actual L2912 + L3070 + L1784) — agent corrected per authorization. Both `</>` and `</NAME>` closers accepted (A1b enforces name-match). 2 `TODO[step-11.0a]` memorials flipped (BRIEF's "7" was cross-step total: 2 + 1 + 2 + 2). **11.0b confirmed independent of 11.0a** — `compoundBody` flag in `collectExpr` already handles children-on-separate-lines via `<` next-token termination. +8 tests.

**Planning durables landed S60:**
- **A1b SCOPE-AND-DECOMPOSITION** (`docs/changes/phase-a1b-resolve-type/SCOPE-AND-DECOMPOSITION.md`, ~440 lines). 22 steps B1-B22 in 5 waves: foundational resolver (B1-B4); cell classification + derived dep (B5-B8); validator typer + validity surface (B9-B13); engine typer (B14-B17); cross-cutting (B18-B22). Lock-to-step + step-to-A1a-dep + test-delta tables. 7 ranked open Qs incl. Step 12 ordering (BLOCKING), wave parallelism (HIGH), validator typer placement (MEDIUM), refinement-zone scope (MEDIUM). Self-host parity deferred policy + `phase-a1b-step-bN-<slug>` branch convention.
- **A1c SCOPE-AND-DECOMPOSITION** (`docs/changes/phase-a1c-codegen/SCOPE-AND-DECOMPOSITION.md`, ~470 lines). 23 steps C1-C23 in 6 waves: foundational state-decl emission (C1-C4); reset+validators (C5-C7); validity surface emission (C8-C11); engines (C12-C15); cross-cutting (C16-C22); PIPELINE prose (C23). Lock-emit + step-to-B-dep + test-delta tables. Predicts +260 to +420 tests. 8 ranked open Qs incl. A1b dependency (BLOCKING), runtime library policy single-bundled-vs-split (HIGH).
- **ADR — `reactive-derived-decl` divergence** RATIFIED 2026-05-05 (S60). User verbatim: "ratify the ADR — Option A". Inserted as Step 11.5 in A1a decomposition; sequenced AFTER Step 11 BEFORE Step 12. Spec coherence (§6.6 derived as state-decl shape) + L21 enforcement substrate (one walker on unified kind) + Step 12 leverage.
- **Step 11.0a/b/c BRIEFs** drafted (`docs/changes/phase-a1a-step-11-0{a,b,c}-*/BRIEF.md`). 11.0a landed; 11.0b + 11.0c queued.
