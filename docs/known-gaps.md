...
**Strategic (deep-dive in flight, `emitted-js-parse-gate-invariant-2026-05-29.md`):** all 5 devs' unprompted #1 ask — an emitted-JS parse gate as a compile-time invariant, so the C1/C2/C5/C8 *class* (compile-clean-but-invalid-JS) can never silently ship again. Complementary to the individual fixes.
...
**Bug 46 close-candidate (triage):** R25/S136 filed Bug 46 "tableFor `sortable=`/`selectable=` not implemented (W-ATTR-001)"; R27 + the S140 Bug-59 fix show per-row checkbox + sortable headers now emit wiring — re-check + likely close.
