...
- **Total (S48 close 2026-04-29):** **7,941 pass, 40 skip, 2 fail** (~28,222 expects across 381 test files). The 2 fails are pre-existing (Bootstrap L3 timeout + tokenizer self-host parity).
...
- [x][ ] **if/show split** (S48 — fix-the-cracks pass). Tutorial-vs-spec-vs-impl 3-way drift surfaced by `tutorial-freshness-audit-2026-04-29.md`. **Phase 1** ✅ S48 (`9873e0e`) — `show=` shipped as visibility-toggle directive (see §M #55). **Phase 2 in progress:** convert `if=` to true mount/unmount per SPEC §17.1 / §6.7.2. Sub-phases: 2a ✅ runtime helpers + `isMountToggle` flag (`90f8d16`); 2b ⚠️ emit-html early-out written + DEFERRED to 2c (`e62a11f` — block commented out; activating fails 22 existing tests that lock in old display-toggle shape — bundle test churn into one disciplined Phase 2c commit); 2c re-enable 2b + update 22 tests (NOT STARTED); 2d events inside if= re-attach per mount cycle; 2e reactive interp inside if= rewires per mount; 2f lifecycle (`on mount`, cleanup) per cycle; 2g IfChainExpr (else / else-if) chooses template; 2h sample-suite verification (15+ files using if=). User-confirmed sequencing: Phase 1 → Phase 2 (full) → 3-5k LOC dispatch app.
