...
| **11.0b** | **Newline-as-statement-separator** inside `parseLogicBody` for state-decl RHS — multi-decl with newline-only separator stops eating sibling decls into the first decl's init string | `ast-builder.js` (parseLogicBody) | **1-2 h** | ⏸ NEXT (BRIEF drafted; INDEPENDENT of 11.0a per agent finding) |
| **11.0c** | **Typed-decl recognizer** — `<count>: number = 0`, `<userInfo>: UserInfo = (...)` — `>` followed by `:` falls through to html-fragment today | `ast-builder.js` (tryParseStructuralDecl) | **2-3 h** | ⏸ (BRIEF drafted) |
