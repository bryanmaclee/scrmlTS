...
| C7 | MED | OPEN | errorBoundary `fallback={<markup/>}` (SPEC §19.6) compiles but emits an inert anchor — ZERO runtime catch wiring | runtime-dead. Ties to the deferred R24 step-3b errorBoundary direction-call. PRIMER/kickstarter `renders=.Fallback` shape doesn't compile (E-ATTR-001) → canon should migrate to `fallback={}` AND the runtime wiring needs building. |
...
**GATE-FOUND (S141, blocks gate always-on) — the emitted-JS parse-gate's first-run discovery:**
- **C10 (HIGH)** compound-predicate `if=(X is some && X != "")` lowering TRUNCATES the `!= ""` operand → a dangling `!==` with no RHS → invalid JS at exit-0. **Dominant cluster** of the gate's 16-artifact find; hits any adopter using the `is some && != ""` compound `if=` shape (not just examples — it's the codegen lowering). Found in `examples/23-trucking-dispatch` (board/billing/drivers/loads/home/load-detail `.client.js`).
- **C11 (MED)** leaked unlowered `server {` block in `examples/23-trucking-dispatch/seeds.server.js` → invalid JS.

*The ~16 invalid-JS artifacts (~12 in trucking-dispatch + ~4 across other examples) are TRUE positives the suite ships green today (examples are compiled, not JS-parse-checked). A fix-wave to close them is the prerequisite to flipping `validateEmit` default ON. **Carry-forward** (CARRY-FORWARD candidate next session).*
