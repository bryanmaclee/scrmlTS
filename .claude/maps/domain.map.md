...
| A-4 per-route artifact splitter | A-4.1 + A-4.2 + A-4.3 + A-4.5 closed S91 — `compiler/src/codegen/route-splitter.ts` (orchestrator + initial-chunk + tier-1 composers) + `compiler/src/codegen/atom-emitter.ts` (per-id atom helpers) + `_scrml_prefetch_tier1` + `_scrml_fetch_chunk` runtime in `compiler/src/runtime-template.js`. Opt-in via `--emit-per-route` CLI flag. A-4.4 / A-4.6 / A-4.7 pending. Test anchors: `compiler/tests/unit/codegen-route-splitter.test.js` (43 tests), `compiler/tests/unit/codegen-route-splitter-tier-n.test.js` (14 tests — A-4.5 runtime + tree-shake + chunk-position), `compiler/tests/integration/initial-chunk-emission.test.js` (16 tests — §40.9.9 worked-example replay), `compiler/tests/integration/tier1-idle-prefetch.test.js` (9 tests — §40.9.9 empty-tier-1 normative + tree-shake DEAD verification) |
...
| A-4.5 tier-N on-demand dispatch runtime | `_scrml_fetch_chunk(epId, role, tier)` rides INSIDE the existing `prefetch` chunk in `compiler/src/runtime-template.js` (between `_scrml_prefetch_tier1` and the §22.5 meta.emit boundary marker). Returns `Promise<string>` via `fetch().text()` for registered (epId, role, tier) tuples; returns JS `null` (canonical scrml absence per §42.5/§42.8) for unregistered tuples. `detectRuntimeChunks` extends the gate so it also lights up on non-empty `prefetchTierN` admission — branch never fires in v0.3 (RS emits empty tier-N per OQ-A2-B Option a + OQ-A4-D Option a), structural-scaffolding for v0.4+. Tree-shake invariant intact: chunk omitted when BOTH tier-1 and tier-N admission are empty |
