...
Current baseline (2026-05-06 S65 close): **9,019 pass / 44 skip / 1 todo / 0 fail / 9,064 across 447 files** (+78 net from session-open; 0 regressions across 5 parallel compiler dispatches). **parseVariant SHIPPED** as L22 family member #1 (`scrml:data` stdlib export at SPEC §41.13). **A1b foundational steps landed:** B3 `@name` resolution + B5 cell classifier — both with depth-of-survey discounts (counter is now 9). **A+ verdict #1+#2+#3 carry-forward FULLY CLOSED** (debate-04 execution items: did-you-mean: match enrichment + W-LIFECYCLE-CANDIDATE tightening + match-expr-{} canonical Tier-1 doc). **ast-builder grammar fixes** (export function swallow + export * + renamed re-exports). **api.js cross-file stdlib enum re-export gap closed** (Phase 2 follow-up). **Promotion ergonomics Tier A landed** (CLI stub + SPEC §56 + primer + 2 article updates); Tier B (lint detection + AST→AST transformation) properly scoped to 25-41h follow-up dispatch. Next: B4-B22 + serialize (L22 #2) + promotion ergonomics Tier B + A1c C0 when A1b completes.
