...
Current baseline (2026-05-06 S64): **8,941 pass / 44 skip / 1 todo / 0 fail / 8,986 across 440 files**. **Phase A1b Step B2 (E-NAME-COLLIDES-STATE) LANDED** — first lock-firing step in A1b; two-pass design within `symbol-table.ts` (PASS 1 from B1 registers state-decls; PASS 2 NEW traverses for local-decl collisions). Surface much smaller than 4-6h estimate — depth-of-survey discount #6. **Stage 0c.A function-overload deletion LANDED.** **Phase 4d completion sweep LANDED.** **3 debates judged in S64** (debate-02 function-overload deletion confirmed; debate-03 component-overload SPEC track CLOSED; debate-04 switch-stmt stays HARD-ERROR per audit rec 5). **Tier-ladder rungs+stability deep-dive landed.** **Forgotten-surface audit landed + 11 primer amendments + 4 SPEC §34 catalog adds.** Next: B3 (@name resolution).
