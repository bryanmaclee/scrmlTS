...
Current baseline (2026-05-14 S91 CLOSE — landmark 28-commit big-session — A-2 + A-3 waves FULLY CLOSED + 03-contact-book v0.2.x latent bug CLOSED + A-4 wave 6 of 7 sub-phases landed + A-4.7 closer in flight): **TBD pass / TBD skip / TBD todo / TBD fail / TBD files** at HEAD `TBD` (full `bun test`, count after A-4.7 landing). v0.2.6 `efbd1e8` still the shipped baseline.

### 2026-05-14 (S91 CLOSE — DRAFT — big-session: 3 major waves closed + A-4 wave 7-of-7 sub-phases dispatched · 28 PA-commits · 4 cherry-pick recoveries · 4 CWD trap-and-catches · zero substantive --no-verify)
...
A-4 wave closer in flight at session close:
- **A-4.7** — agent V dispatched at S91 close; brief staged at `docs/changes/a-4-7-per-route-html-augmentation/BRIEF.md`. Closes A-4.2's forward-looking gap (NEW `_scrml_chunk_mount` + `_scrml_vendor_require` runtime helpers — chunks become RUNNABLE in adopter browsers for the first time); per-route HTML augmentation (role-detection bootstrap per OQ-A4-E hybrid + `_SCRML_CHUNKS` inline injection + `<link rel="modulepreload">` for tier-1); routeSegmentFromEntryPointId real-pipeline fix per A-4.2 deferred-item flag; NEW W-CG-CHUNK-* lint family (4 codes — EMPTY, LARGE, NO-PREFETCH, MISSING-ROLE) + §34 + §40.9.11 catalog rows. **When A-4.7 lands: A-4 wave FULLY CLOSED → v0.3.0 critical path substantively complete.**
...
**Push state at S91 close:** scrmlTS 28 commits ahead of origin (`199940e..<final>`); scrml-support 1 commit ahead (`8d13012`). Push pending wrap step 7.

**Test count trajectory S91:** 12,275 (S90 close baseline) → 11,697+ (A-4.4 close pre-commit subset, A-4.7 landing will add more). Pre-commit gate ran clean on every substantive commit.
