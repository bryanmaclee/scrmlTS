...
2. **Next priority — choose ONE (per hand-off-75 §"Open questions to surface immediately"):**
   ...
   - **C15 follow-up dispatches.**
     - Codegen-side FileAST-shape divergence (§C15.11/§C15.12) — `collectCrossFileEngineMounts` walker returns empty for real-pipeline FileAST. ~30min-1h fast fix once pinpointed.
     - MOD re-export engine-category fall-through (§C15.13) — `export { phase } from './engines.scrml'` lands as `kind:"re-export"`, falls through. ~3-5h separate dispatch.
   ...
   - **Insight 28 follow-up OQs (4 standing):** OQ-bridge-3 (§53.2.1 grammar list audit — independent ~spec-author touch), OQ-bridge-4 (`safeCompare`-shape audit in `validate.scrml` — 15-30min), OQ-bridge-5 (compile-time WARNING when bridged validator on schema-column field — defer to compiler-diagnostics audit), OQ-bridge-2 (re-debate trigger on ≥3 adopter friction reports — passive).
